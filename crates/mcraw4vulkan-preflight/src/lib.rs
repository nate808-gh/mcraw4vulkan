...
mod tests {
    ...
    fn passing_probe() -> ProbeSnapshot {
        ProbeSnapshot {
            platform: PlatformSummary {
                kind: PlatformKind::Linux,
                os_summary: "Linux Test Distro kernel 6.1.0".to_string(),
                mount_backend: MountBackendKind::LinuxFuse3,
                mount_backend_status: PreflightRequirementStatus::Available,
                mount_backend_detail: "test mount backend".to_string(),
            },
            hardware: HardwareSummary {
            ...
            vulkan: VulkanSummary {
                ...
                adapter_name: Some("Test Vulkan GPU".to_string()),
                detail: "test Vulkan adapter".to_string(),
            },
        }
    }

    fn report_for(options: PreflightOptions, probe: ProbeSnapshot) -> PreflightReport {
        report_from_probe(options, probe, &mut |_| {})
    }

    #[test]
    fn emits_preflight_initial_line() {
        let report = report_for(PreflightOptions::gui_blocking_policy(), passing_probe());

        assert_eq!(
            report.events.first().map(|event| event.line.as_str()),
            Some("Pre-flight Checklist in progress")
        );
    }

    #[test]
    fn gui_policy_passes_when_mount_and_vulkan_pass() {
        let report = report_for(PreflightOptions::gui_blocking_policy(), passing_probe());

        assert_eq!(report.status, PreflightStatus::ReadyToLaunch);
        assert!(report.ready_to_launch);
        assert!(
            report
                .events
                .iter()
                .any(|event| event.line == "Launching mcraw4vulkan")
        );
    }

    #[test]
    fn gui_policy_blocks_when_mount_is_missing() {
        let mut probe = passing_probe();
        probe.platform.mount_backend_status = PreflightRequirementStatus::Missing;

        let report = report_for(PreflightOptions::gui_blocking_policy(), probe);
        ...
        assert!(
            report
                .events
                .iter()
                .any(|event| event.line == "FUSE3 is required")
        );
        assert!(
            !report
                .events
    ...
    #[test]
    fn gui_policy_blocks_when_vulkan_is_missing() {
        let mut probe = passing_probe();
        probe.vulkan.status = PreflightRequirementStatus::Missing;
        ...
        assert!(
            report
                .events
                .iter()
                .any(|event| event.line == "Vulkan libraries are required")
        );
    }

    #[test]
    fn cli_informational_policy_does_not_block_on_missing_requirements() {
        let mut probe = passing_probe();
        probe.platform.mount_backend_status = PreflightRequirementStatus::Missing;
        probe.vulkan.status = PreflightRequirementStatus::Missing;

        let report = report_for(PreflightOptions::cli_informational_policy(), probe);
    ...
    #[test]
    fn event_sink_receives_lines_in_order() {
        let mut lines = Vec::new();
        let report = report_from_probe(
            PreflightOptions::gui_blocking_policy(),
            passing_probe(),
            &mut |event| lines.push(event.line.clone()),
        );
        ...
        let report_lines = report
            ...
            .collect::<Vec<_>>();
        assert_eq!(lines, report_lines);
        assert_eq!(
            lines,
            vec![
                "Pre-flight Checklist in progress",
                "Linux Test Distro kernel 6.1.0",
                "CPU Test CPU",
                "GPU Test GPU",
                "FUSE3: found",
                "Vulkan: found",
                "Launching mcraw4vulkan",
            ]
        );
    }

    #[test]
    fn vulkan_adapter_can_supply_gpu_model() {
        let mut probe = passing_probe();
        probe.hardware.gpu_model = None;
        let mut options = PreflightOptions::gui_blocking_policy();
        options.probe_gpu_model = true;
        ...
        let report = report_for(options, probe);

        assert_eq!(
            report.hardware.gpu_model.as_deref(),
            Some("Test Vulkan GPU")
        );
        assert!(
            report
                .events
                .iter()
                .any(|event| event.line == "GPU Test Vulkan GPU")
        );
    }
}
