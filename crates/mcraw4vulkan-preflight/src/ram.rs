...
mod tests {
    ...
    #[test]
    fn formats_whole_number_ram_line() {
        let snapshot = SystemRamSnapshot {
            installed_bytes: Some(128 * 1024 * 1024 * 1024),
            available_bytes: Some(84 * 1024 * 1024 * 1024),
        };

        assert_eq!(
            format_system_ram_line(&snapshot),
            "Installed RAM 128 GB | Available RAM 84 GB"
        );
    }

    #[test]
    fn formats_unknown_ram_values() {
        assert_eq!(
            format_system_ram_line(&SystemRamSnapshot::unknown()),
            "Installed RAM unknown | Available RAM unknown"
        );
    }

    #[test]
    fn rounds_binary_gib_to_nearest_display_gb() {
        let snapshot = SystemRamSnapshot {
            installed_bytes: Some(3 * 1024 * 1024 * 1024 + 600 * 1024 * 1024),
            available_bytes: None,
        };

        assert_eq!(
            format_system_ram_line(&snapshot),
            "Installed RAM 4 GB | Available RAM unknown"
        );
    }

    #[test]
    fn parses_linux_meminfo_total_and_available() {
        let input = "\
MemTotal:       134217728 kB
MemFree:         10000000 kB
MemAvailable:    88080384 kB
";

        assert_eq!(
            parse_linux_meminfo(input),
            SystemRamSnapshot {
                installed_bytes: Some(128 * 1024 * 1024 * 1024),
                available_bytes: Some(84 * 1024 * 1024 * 1024),
            }
    ...
    #[test]
    fn linux_meminfo_falls_back_to_memfree() {
        let input = "\
MemTotal:       134217728 kB
MemFree:          1048576 kB
";
        ...
        assert_eq!(
            parse_linux_meminfo(input).available_bytes,
            Some(1024 * 1024 * 1024)
        );
    }

    #[test]
    fn parses_macos_vm_stat_available_pages() {
        let input = "\
...
";
        ...
        assert_eq!(parse_macos_vm_stat_available(input), Some(3500 * 16384));
    }

    #[test]
    fn parses_macos_hw_memsize() {
        assert_eq!(
            parse_macos_hw_memsize("137438953472\n"),
            Some(128 * 1024 * 1024 * 1024)
        );
    }

    #[test]
    fn parses_windows_powershell_memory_kib() {
        assert_eq!(
            parse_windows_powershell_memory("134217728\r\n88080384\r\n"),
            Some(SystemRamSnapshot {
                installed_bytes: Some(128 * 1024 * 1024 * 1024),
                available_bytes: Some(84 * 1024 * 1024 * 1024),
            })
    ...
    #[test]
    fn parses_windows_wmic_memory_kib() {
        let input = "\
FreePhysicalMemory=88080384
TotalVisibleMemorySize=134217728
";

        assert_eq!(
            parse_windows_wmic_memory(input),
            Some(SystemRamSnapshot {
                installed_bytes: Some(128 * 1024 * 1024 * 1024),
                available_bytes: Some(84 * 1024 * 1024 * 1024),
            })
