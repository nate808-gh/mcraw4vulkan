...
mod tests {
    ...
    #[test]
    fn package_root_is_detected_only_from_package_bin_dir() {
        let root = PathBuf::from("tmp/mcraw4vulkan-macos-cli");
        let release_binary =
            absolute_path(&["tmp", "target", "release", "mcraw4vulkan-preflight-check"]);

        assert_eq!(
            package_root_from_exe_path(&root.join("bin/mcraw4vulkan-preflight-macos")),
            None
        );
        assert_eq!(package_root_from_exe_path(&release_binary), None);
    }

    #[test]
    fn macos_package_probe_requires_wrapper_icd_environment() {
        let root = env::temp_dir().join(format!(
            "mcraw4vulkan-preflight-vulkan-test-{}",
            std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(root.join("lib")).expect("create lib");
        std::fs::create_dir_all(root.join("vulkan/icd.d")).expect("create icd");
        for path in macos_package_vulkan_paths(&root) {
            std::fs::write(path, b"").expect("write package runtime file");
        }

        let without_env = probe_macos_package_vulkan(&root, None);
        let with_env = probe_macos_package_vulkan(
            &root,
            Some(root.join("vulkan/icd.d/MoltenVK_icd.json").as_os_str()),
        );

        assert_eq!(without_env.status, PreflightRequirementStatus::Missing);
        assert_eq!(with_env.status, PreflightRequirementStatus::Available);

        let _ = std::fs::remove_dir_all(&root);
    }

    #[test]
    fn linux_vulkan_probe_reports_missing_from_empty_inputs() {
        let missing_path = absolute_path(&["does", "not", "exist"]);
        let summary = probe_linux_vulkan(None, Some(missing_path.as_os_str()));

        if summary.status == PreflightRequirementStatus::Missing {
            assert!(summary.detail.contains("libvulkan.so.1"));
        }
    }

    #[test]
    fn windows_vulkan_probe_uses_system_root() {
        let missing_root = PathBuf::from(["Z:", "definitely-missing"].join("\\"));
        let summary = probe_windows_vulkan(Some(missing_root.as_os_str()));

        assert_eq!(summary.status, PreflightRequirementStatus::Missing);
        assert!(summary.detail.contains("vulkan-1.dll"));
    }
}
