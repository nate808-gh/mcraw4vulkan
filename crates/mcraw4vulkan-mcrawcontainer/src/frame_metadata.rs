...
mod tests {
    ...
    #[test]
    fn parses_pixel_and_non_pixel_lens_shading_map_dimensions() {
        let pixel_metadata = frame_metadata_json_with_lens_shading_map(33, 25, 825);
        let pixel = FrameMetadata::parse(&pixel_metadata).expect("valid 33x25 map parses");
        let pixel_map = pixel.lens_shading_map.expect("lens map is present");
        assert_eq!(pixel_map.width(), 33);
        assert_eq!(pixel_map.height(), 25);
        assert_eq!(pixel_map.plane_count(), 4);
        assert_eq!(pixel_map.expected_plane_len(), 825);

        let non_pixel_metadata = frame_metadata_json_with_lens_shading_map(17, 13, 221);
        let non_pixel = FrameMetadata::parse(&non_pixel_metadata).expect("valid 17x13 map parses");
        let non_pixel_map = non_pixel.lens_shading_map.expect("lens map is present");
        assert_eq!(non_pixel_map.width(), 17);
        assert_eq!(non_pixel_map.height(), 13);
        assert_eq!(non_pixel_map.plane_count(), 4);
        assert_eq!(non_pixel_map.expected_plane_len(), 221);
    }

    #[test]
    fn rejects_lens_shading_map_plane_length_mismatch() {
        let metadata = frame_metadata_json_with_lens_shading_map(17, 13, 220);
        let error = FrameMetadata::parse(&metadata).expect_err("wrong plane length is rejected");

        assert!(error.to_string().contains("len 220 does not match"));
    }

    #[test]
    fn detects_binned_raw16_type6_layout() {
        ...
        let metadata = FrameMetadata::parse(&value.to_string()).expect("metadata parses");

        assert_eq!(metadata.compression_type, CompressionType::MotionCamType6);
        assert_eq!(metadata.compression_type_raw.as_deref(), Some("6"));
        assert_eq!(metadata.is_binned, Some(true));
        assert_eq!(metadata.is_compressed, Some(true));
        assert_eq!(metadata.need_remosaic, Some(false));
        assert_eq!(
            metadata
                .payload_layout()
                .expect("type 6 layout is supported"),
            FramePayloadLayout::BinnedRaw16Type6 { row_stride: 3840 }
        );
    }

    #[test]
    fn detects_compressed_rawcodec_type7_layout() {
        let value = serde_json::json!({
            "width": 4080,
            "height": 3072,
            "compressionType": 7,
        ...
        assert_eq!(metadata.compression_type, CompressionType::MotionCamType7);
        assert_eq!(
            metadata
                .payload_layout()
                .expect("type 7 layout is supported"),
            FramePayloadLayout::CompressedRawcodecType7
        );
    }

    #[test]
    fn unsupported_layout_diagnostic_includes_key_fields() {
        let value = serde_json::json!({
            "width": 1920,
            "height": 1080,
            "rowStride": 3840,
            "pixelFormat": "unknown-format",
            "isBinned": true,
            "compressionType": 99,
        });
        let metadata = FrameMetadata::parse(&value.to_string()).expect("metadata parses");
        let error = metadata
            .payload_layout()
            .expect_err("unsupported type returns diagnostic");
        let message = error.to_string();

        assert!(message.contains("compressionType 99"));
        assert!(message.contains("pixelFormat unknown-format"));
        assert!(message.contains("isBinned true"));
        assert!(message.contains("rowStride 3840"));
        assert!(message.contains("dimensions 1920x1080"));
    }

    fn frame_metadata_json_with_lens_shading_map(
        map_width: u32,
        map_height: u32,
        plane_len: usize,
    ) -> String {
        let plane = vec![1.0_f32; plane_len];
        let value = serde_json::json!({
            "width": 4,
            "height": 4,
            "lensShadingMapWidth": map_width,
            "lensShadingMapHeight": map_height,
            "lensShadingMap": [
                plane.clone(),
                plane.clone(),
                plane.clone(),
