...
mod tests {
    ...
    #[test]
    fn plan_detects_monotonic_offsets() {
        let plan = plan(vec![entry(0, 0, 4), entry(1, 4, 4), entry(2, 8, 4)]);
        assert!(plan.monotonic_offsets);
        assert_eq!(plan.backward_seek_count, 0);
        assert_eq!(plan.contiguous_pair_count, 2);
    }

    #[test]
    fn plan_detects_non_monotonic_playback_offsets() {
        let plan = plan(vec![entry(0, 20, 4), entry(1, 0, 4), entry(2, 10, 4)]);
        assert!(!plan.monotonic_offsets);
        assert_eq!(plan.backward_seek_count, 1);
        assert_eq!(plan.non_monotonic_pairs, 1);
        assert_eq!(plan.max_reorder_distance_frames, 2);
    }

    #[test]
    fn chunk_grouping_honors_gap_threshold() {
        let plan = plan(vec![entry(0, 0, 4), entry(1, 6, 4), entry(2, 40, 4)]);
        let stats = plan
            .chunk_layout_stats(PayloadChunkOptions::new(4, 128, 8))
            .unwrap();
        assert_eq!(stats.chunk_count, 2);
        assert_eq!(stats.chunk_payload_bytes, 12);
        assert_eq!(stats.chunk_gap_bytes_read, 2);
    }

    #[test]
    fn chunk_grouping_honors_max_bytes() {
        let plan = plan(vec![entry(0, 0, 10), entry(1, 12, 10)]);
        let stats = plan
            .chunk_layout_stats(PayloadChunkOptions::new(8, 16, 8))
            .unwrap();
        assert_eq!(stats.chunk_count, 2);
    }

    #[test]
    fn chunk_grouping_honors_max_payloads() {
        let plan = plan(vec![entry(0, 0, 4), entry(1, 4, 4), entry(2, 8, 4)]);
        let stats = plan
            .chunk_layout_stats(PayloadChunkOptions::new(8, 128, 2))
            .unwrap();
        assert_eq!(stats.chunk_count, 2);
        assert_eq!(stats.chunk_max_payloads_observed, 2);
    }

    #[test]
    fn chunk_slice_accessors_match_bytes() {
        let chunk: Arc<[u8]> = Arc::from(vec![1, 2, 3, 4, 5].into_boxed_slice());
        let data = PayloadFrameData::ChunkSlice { chunk, range: 1..4 };
        assert_eq!(data.as_slice(), &[2, 3, 4]);
        let mut copied = Vec::new();
        data.copy_into(&mut copied);
        assert_eq!(copied, &[2, 3, 4]);
        assert_eq!(data.into_owned(), &[2, 3, 4]);
    }

    #[test]
    fn owned_payload_accessors_reuse_and_move_storage() {
        let data = PayloadFrameData::Owned(vec![7, 8, 9]);
        assert_eq!(data.backing_kind(), "owned");
        assert_eq!(data.into_owned(), vec![7, 8, 9]);

        let data = PayloadFrameData::Owned(vec![1, 2, 3, 4]);
        let mut copied = Vec::with_capacity(64);
        let capacity_before = copied.capacity();
        data.copy_into(&mut copied);
        assert_eq!(copied, vec![1, 2, 3, 4]);
        assert_eq!(copied.capacity(), capacity_before);
    }

    #[test]
    fn production_default_is_low_read_chunked_policy() {
        let options = PayloadFeederOptions::production_default();
        assert_eq!(options.mode, PayloadReadMode::ChunkedOffsetPrefetch);
        assert_eq!(options.prefetch_depth, 4);
        assert_eq!(options.reorder_window, 128);
        assert_eq!(
            options.chunk_options,
            PayloadChunkOptions {
                gap_threshold_bytes: NEAR_CONTIGUOUS_THRESHOLD_BYTES,
                max_chunk_bytes: 64 * 1024 * 1024,
                max_payloads_per_chunk: 16,
            }
        );
        assert_eq!(PayloadFeederOptions::default(), options);
    }

    #[test]
    fn chunked_feeder_emits_playback_order_chunk_slices() {
        let path = write_temp(&[10, 11, 12, 13, 20, 21, 22, 23, 30, 31, 32, 33]);
        let plan = plan(vec![entry(0, 8, 4), entry(1, 0, 4), entry(2, 4, 4)]);
        let mut feeder = PayloadFeeder::spawn(
            &path,
            plan,
            PayloadFeederOptions {
                mode: PayloadReadMode::ChunkedOffsetPrefetch,
                prefetch_depth: 2,
                reorder_window: 4,
                chunk_options: PayloadChunkOptions::new(16, 64, 8),
            },
        )
        .unwrap();

        let first = feeder.next_frame().unwrap().unwrap();
        let second = feeder.next_frame().unwrap().unwrap();
        let third = feeder.next_frame().unwrap().unwrap();
        assert_eq!(first.frame_number, 0);
        assert_eq!(first.data.as_slice(), &[30, 31, 32, 33]);
        assert_eq!(first.data.backing_kind(), "chunk-slice");
        assert_eq!(second.frame_number, 1);
        assert_eq!(second.data.as_slice(), &[10, 11, 12, 13]);
        assert_eq!(third.frame_number, 2);
        assert_eq!(third.data.as_slice(), &[20, 21, 22, 23]);
        assert!(feeder.next_frame().unwrap().is_none());
        let stats = feeder.finish().unwrap();
        assert!(stats.chunked_reads);
        assert_eq!(stats.frame_data_backing(), "chunk-slice");
        fs::remove_file(path).ok();
    }

    #[test]
    fn resequence_window_exceeded_is_reported() {
        let path = write_temp(&[10, 11, 12, 13, 20, 21, 22, 23, 30, 31, 32, 33]);
        let plan = plan(vec![entry(0, 8, 4), entry(1, 0, 4), entry(2, 4, 4)]);
        let mut feeder = PayloadFeeder::spawn(
            &path,
            plan,
            PayloadFeederOptions {
                mode: PayloadReadMode::OffsetPrefetch,
                prefetch_depth: 1,
                reorder_window: 1,
                chunk_options: PayloadChunkOptions::default(),
            },
        )
        .unwrap();
        let error = feeder.next_frame().unwrap_err().to_string();
        assert!(error.contains("resequence window exceeded"));
        fs::remove_file(path).ok();
    }

    #[test]
    fn early_drop_cancels_worker() {
        let path = write_temp(&[0u8; 64]);
        let plan = plan((0..8).map(|i| entry(i, (i * 8) as u64, 8)).collect());
        let feeder = PayloadFeeder::spawn(
            &path,
            plan,
            PayloadFeederOptions {
                mode: PayloadReadMode::OffsetPrefetch,
    ...
    #[test]
    fn adaptive_selects_current_for_one_frame() {
        let decision = adaptive(
            plan(vec![entry(0, 0, 4)]),
            PayloadAdaptivePolicy::Throughput,
            PayloadChunkOptions::new(8, 128, 8),
            8,
            1024,
        );
        assert_eq!(decision.selected, PayloadReadMode::Current);
        assert_eq!(decision.reason, PayloadAdaptiveReason::SingleFrame);
    }

    #[test]
    fn adaptive_throughput_selects_offset_for_normal_multiframe_plan() {
        let decision = adaptive(
            plan((0..16).map(|i| entry(i, (i * 16) as u64, 4)).collect()),
            PayloadAdaptivePolicy::Throughput,
            PayloadChunkOptions::new(8, 128, 8),
            8,
            1024,
        );
        assert_eq!(decision.selected, PayloadReadMode::OffsetPrefetch);
        assert_eq!(
            decision.reason,
            PayloadAdaptiveReason::ThroughputOffsetDefault
        );
    ...
    #[test]
    fn adaptive_balanced_selects_chunked_when_reduction_is_high() {
        let decision = adaptive(
            plan((0..16).map(|i| entry(i, (i * 4) as u64, 4)).collect()),
            PayloadAdaptivePolicy::Balanced,
            PayloadChunkOptions::new(0, 1024, 32),
            8,
            1024,
        );
        assert_eq!(decision.selected, PayloadReadMode::ChunkedOffsetPrefetch);
        assert_eq!(decision.reason, PayloadAdaptiveReason::BalancedChunked);
        assert!(decision.estimated_read_reduction >= 16.0);
    }

    #[test]
    fn adaptive_fewest_reads_selects_chunked_when_allowed() {
        let decision = adaptive(
            plan((0..4).map(|i| entry(i, (i * 4) as u64, 4)).collect()),
            PayloadAdaptivePolicy::FewestReads,
            PayloadChunkOptions::new(0, 1024, 32),
            8,
            1024,
        ...
        assert_eq!(decision.selected, PayloadReadMode::ChunkedOffsetPrefetch);
        assert_eq!(decision.reason, PayloadAdaptiveReason::FewestReadsChunked);
    }

    #[test]
    fn adaptive_falls_back_when_chunk_overread_exceeds_threshold() {
        let decision = adaptive(
            plan(vec![entry(0, 0, 4), entry(1, 100, 4)]),
            PayloadAdaptivePolicy::Balanced,
            PayloadChunkOptions::new(128, 1024, 32),
            8,
            1,
        );
        assert_eq!(decision.selected, PayloadReadMode::OffsetPrefetch);
        ...
        assert!(!decision.candidate_chunked_allowed);
    }

    #[test]
    fn adaptive_falls_back_when_retained_chunk_bytes_exceeds_limit() {
        let plan = plan(vec![entry(0, 0, 4), entry(1, 4, 4)]);
        let decision = PayloadAdaptiveDecision::resolve(
            PayloadReadModeRequest::Adaptive,
            &plan,
            PayloadFeederOptions {
                mode: PayloadReadMode::Current,
                prefetch_depth: 4,
                reorder_window: 128,
                chunk_options: PayloadChunkOptions::new(0, 1024, 32),
            },
            PayloadAdaptiveOptions {
                policy: PayloadAdaptivePolicy::FewestReads,
                max_retained_chunk_bytes: 4,
    ...
    #[test]
    fn adaptive_falls_back_when_reorder_window_is_too_small() {
        let plan = plan(vec![entry(0, 20, 4), entry(1, 0, 4), entry(2, 8, 4)]);
        let decision = PayloadAdaptiveDecision::resolve(
            PayloadReadModeRequest::Adaptive,
            &plan,
            PayloadFeederOptions {
    ...
    #[test]
    fn adaptive_handles_empty_plan() {
        let decision = adaptive(
            PayloadReadPlan::empty(),
            PayloadAdaptivePolicy::Balanced,
            PayloadChunkOptions::new(0, 1024, 32),
            8,
            1024,
        ...
        assert_eq!(decision.selected, PayloadReadMode::Current);
        assert_eq!(decision.reason, PayloadAdaptiveReason::EmptyPlan);
    }

    fn plan(entries: Vec<PayloadFrameRequest>) -> PayloadReadPlan {
        PayloadReadPlan::from_requests(entries).unwrap()
    }

    fn adaptive(
        plan: PayloadReadPlan,
        policy: PayloadAdaptivePolicy,
        chunk_options: PayloadChunkOptions,
        max_overread_ratio: u64,
        min_read_reduction_milli: u64,
    ) -> PayloadAdaptiveDecision {
        PayloadAdaptiveDecision::resolve(
            PayloadReadModeRequest::Adaptive,
            &plan,
            PayloadFeederOptions {
                mode: PayloadReadMode::Current,
            ...
            PayloadAdaptiveOptions {
                policy,
                max_overread_ratio: max_overread_ratio as f64,
                min_read_reduction: min_read_reduction_milli as f64 / 1000.0,
                ..PayloadAdaptiveOptions::default()
            },
        )
        .unwrap()
    ...
    fn entry(frame_number: usize, offset: u64, len: usize) -> PayloadFrameRequest {
        PayloadFrameRequest {
            playback_index: frame_number,
            frame_number,
    ...
    fn write_temp(bytes: &[u8]) -> PathBuf {
        ...
        let path = std::env::temp_dir().join(format!(
            "mcraw4vulkan-payload-reader-test-{}-{id}.bin",
            std::process::id()
        ));
