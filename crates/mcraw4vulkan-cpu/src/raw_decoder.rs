...
#[cfg(test)]
mod tests {
    use mcraw4vulkan_core::{BlockEncoding, FrameDimensions, FramePayloadLayout};

    use super::{
        PreparedRawDecodeBlock, PreparedRawDecodeMetadata, RawDecodeError,
        decode_binned_raw16_payload_into, decode_binned_raw16_payload_into_le_bytes,
        decode_frame_payload_to_decoded_bayer_u16_frame, decode_prepared_raw_payload_into,
        decode_prepared_raw_payload_into_le_bytes,
        decode_prepared_raw_payload_to_decoded_bayer_u16_frame, decode_raw_payload_into,
        decode_raw_payload_into_le_bytes, decode_raw_payload_to_decoded_bayer_u16_frame,
        validate_binned_raw16_payload,
    };
    use mcraw4vulkan_rawcodec::{ENCODING_BLOCK, METADATA_OFFSET};

    #[test]
    fn prepared_le_byte_decode_matches_prepared_u16_decode() {
        let raw_payload = [];
        let dimensions = [
            FrameDimensions {
                width: 64,
        ...
        let blocks = zero_encoded_reference_blocks();

        for visible_dimensions in dimensions {
            let prepared_metadata = PreparedRawDecodeMetadata {
                encoded_dimensions: FrameDimensions {
                    width: 64,
                    height: 4,
                },
            ...
            let pixel_count = visible_dimensions
                .pixel_count()
                .expect("synthetic dimensions fit usize");
            let mut pixels = vec![0; pixel_count];
            let mut pixel_bytes_le = vec![0xff; 3];

            let legacy_info = decode_prepared_raw_payload_into(
                &raw_payload,
                visible_dimensions,
                prepared_metadata,
                &mut pixels,
            )
            .expect("prepared u16 decode succeeds");
            let byte_info = decode_prepared_raw_payload_into_le_bytes(
                &raw_payload,
                visible_dimensions,
                prepared_metadata,
                &mut pixel_bytes_le,
            )
            .expect("prepared little-endian byte decode succeeds");

            let mut expected_bytes = Vec::with_capacity(pixel_count * 2);
            for pixel in &pixels {
                expected_bytes.extend_from_slice(&pixel.to_le_bytes());
            }

            assert_eq!(legacy_info.encoded_dimensions, byte_info.encoded_dimensions);
            assert_eq!(legacy_info.visible_dimensions, byte_info.visible_dimensions);
            assert_eq!(pixel_bytes_le, expected_bytes);
        }
    }

    #[test]
    fn prepared_decoded_bayer_u16_frame_matches_direct_le_byte_decode() {
        let raw_payload = [];
        let visible_dimensions = FrameDimensions {
            width: 4,
            height: 4,
        };
        let blocks = zero_encoded_reference_blocks();
        ...
        let mut direct_pixel_bytes_le = Vec::new();

        let direct_info = decode_prepared_raw_payload_into_le_bytes(
            &raw_payload,
            visible_dimensions,
            prepared_metadata,
            &mut direct_pixel_bytes_le,
        )
        .expect("prepared direct little-endian byte decode succeeds");
        let (frame, frame_info) = decode_prepared_raw_payload_to_decoded_bayer_u16_frame(
            &raw_payload,
            visible_dimensions,
            prepared_metadata,
        )
        .expect("prepared shared decoded Bayer U16 frame decode succeeds");

        assert_eq!(frame.dimensions(), visible_dimensions);
        assert_eq!(frame.pixel_bytes_le(), direct_pixel_bytes_le);
        assert_eq!(
            frame_info.encoded_dimensions,
            direct_info.encoded_dimensions
        );
        assert_eq!(
            frame_info.visible_dimensions,
            direct_info.visible_dimensions
        );
    }

    #[test]
    fn le_byte_decode_matches_legacy_u16_decode() {
        let raw_payload = synthetic_zero_encoded_payload();
        let dimensions = [
            FrameDimensions {
                width: 64,
                height: 4,
        ...
        for visible_dimensions in dimensions {
            ...
            let legacy_info =
                decode_raw_payload_into(&raw_payload, visible_dimensions, &mut pixels)
                    .expect("legacy u16 decode succeeds");
            let byte_info = decode_raw_payload_into_le_bytes(
                &raw_payload,
                visible_dimensions,
                &mut pixel_bytes_le,
            )
            .expect("little-endian byte decode succeeds");
    ...
    #[test]
    fn decoded_bayer_u16_frame_matches_direct_le_byte_decode() {
        let raw_payload = synthetic_zero_encoded_payload();
        let visible_dimensions = FrameDimensions {
            width: 4,
            height: 4,
        ...
        let direct_info = decode_raw_payload_into_le_bytes(
            &raw_payload,
            visible_dimensions,
            &mut direct_pixel_bytes_le,
        )
        .expect("direct little-endian byte decode succeeds");
        let (frame, frame_info) =
            decode_raw_payload_to_decoded_bayer_u16_frame(&raw_payload, visible_dimensions)
                .expect("shared decoded Bayer U16 frame decode succeeds");
    ...
    #[test]
    fn binned_raw16_tight_2x2_preserves_sample_values() {
        let dimensions = FrameDimensions {
            width: 2,
            height: 2,
        };
        let payload = legacy_raw16_payload_for_rows(&[&[0, 1], &[1022, 1023]]);
        let mut pixels = vec![0; 4];
        let info = decode_binned_raw16_payload_into(&payload, dimensions, 4, &mut pixels)
            .expect("tight raw16 payload decodes");

        assert_eq!(
            info.encoded_dimensions,
            FrameDimensions {
                width: 32,
                height: 2,
            }
        );
        assert_eq!(info.visible_dimensions, dimensions);
        assert_eq!(pixels, vec![0, 1, 1022, 1023]);
    }

    #[test]
    fn binned_raw16_padded_encoded_rows_copy_visible_samples_only() {
        let dimensions = FrameDimensions {
            width: 4,
            height: 2,
        };
        let payload = legacy_raw16_payload_for_rows(&[&[1, 2, 3, 4], &[5, 6, 7, 8]]);

        let mut pixel_bytes = Vec::new();
        decode_binned_raw16_payload_into_le_bytes(&payload, dimensions, 8, &mut pixel_bytes)
            .expect("padded raw16 payload decodes");

        assert_eq!(pixel_bytes, raw16_bytes(&[1, 2, 3, 4, 5, 6, 7, 8]));
    }

    #[test]
    fn binned_raw16_rejects_short_row_stride() {
        let dimensions = FrameDimensions {
            width: 4,
            height: 2,
        };
        let error = validate_binned_raw16_payload(&[], dimensions, 7)
            .expect_err("rowStride below width * 2 is rejected");

        assert_eq!(
            error,
            RawDecodeError::Raw16RowStrideTooShort {
                row_stride: 7,
                minimum: 8,
            }
        );
    ...
    #[test]
    fn binned_raw16_rejects_short_payload() {
        let dimensions = FrameDimensions {
            width: 4,
            height: 2,
        };
        let mut payload = vec![0xa0, 0x00];
        payload.extend_from_slice(&[0; 10]);
        let error = validate_binned_raw16_payload(&payload, dimensions, 8)
            .expect_err("payload shorter than legacy blocks is rejected");

        assert!(matches!(error, RawDecodeError::Raw16PayloadTooShort { .. }));
    }

    #[test]
    fn layout_aware_binned_raw16_decode_preserves_10_bit_codes() {
        let dimensions = FrameDimensions {
            width: 2,
            height: 2,
        };
        let payload = legacy_raw16_payload_for_rows(&[&[63, 64], &[512, 1023]]);
        let (frame, info) = decode_frame_payload_to_decoded_bayer_u16_frame(
            &payload,
            dimensions,
            FramePayloadLayout::BinnedRaw16Type6 { row_stride: 4 },
        )
        .expect("layout-aware raw16 decode succeeds");
        ...
        assert_eq!(frame.pixel_bytes_le(), raw16_bytes(&[63, 64, 512, 1023]));
    }

    fn zero_encoded_reference_blocks() -> [PreparedRawDecodeBlock; 4] {
        [
            PreparedRawDecodeBlock {
                encoding: BlockEncoding::Zero,
                reference: 100,
            },
            PreparedRawDecodeBlock {
                encoding: BlockEncoding::Zero,
    ...
    fn synthetic_zero_encoded_payload() -> Vec<u8> {
        let bits_offset = METADATA_OFFSET;
        let bits_metadata_len = 4 + 2;
        let refs_offset = bits_offset + bits_metadata_len;

        let mut payload = Vec::new();
        payload.extend_from_slice(&(ENCODING_BLOCK as u32).to_le_bytes());
        payload.extend_from_slice(&4u32.to_le_bytes());
        payload.extend_from_slice(&(bits_offset as u32).to_le_bytes());
        payload.extend_from_slice(&(refs_offset as u32).to_le_bytes());

        debug_assert_eq!(payload.len(), METADATA_OFFSET);

        payload.extend_from_slice(&4u32.to_le_bytes());
        payload.extend_from_slice(&[0, 0]);

        debug_assert_eq!(payload.len(), refs_offset);

        payload.extend_from_slice(&4u32.to_le_bytes());
        payload.extend_from_slice(&[0xf0, 0]);

        for index in 0..ENCODING_BLOCK {
            let value: u16 = match index {
                0 => 100,
                1 => 200,
                2 => 300,
                3 => 400,
    ...
    fn legacy_raw16_payload_for_rows(rows: &[&[u16]]) -> Vec<u8> {
        let mut payload = Vec::new();
        for row in rows {
            let mut even = [0u16; 16];
            let mut odd = [0u16; 16];
            for (col, sample) in row.iter().copied().enumerate() {
                if col % 2 == 0 {
                    even[col / 2] = sample;
                } else {
                    odd[col / 2] = sample;
            ...
            payload.extend_from_slice(&legacy_raw16_block(10, 0, even));
            payload.extend_from_slice(&legacy_raw16_block(10, 0, odd));
        }
        payload
    }

    fn legacy_raw16_block(bits: u8, reference: u16, samples: [u16; 16]) -> Vec<u8> {
        assert!(bits <= 10);
        assert!(reference <= 0x0fff);
        let mut block = Vec::new();
        block.push((bits << 4) | ((reference >> 8) as u8 & 0x0f));
        block.push(reference as u8);
        block.extend_from_slice(&legacy_msb_pack_bits(bits as usize, &samples));
        block
    }

    fn legacy_msb_pack_bits(bits: usize, samples: &[u16; 16]) -> Vec<u8> {
        let mut packed = vec![0u8; (bits * samples.len()).div_ceil(8)];
        let mut bit_offset = 0usize;
        for sample in samples {
            for bit_index in (0..bits).rev() {
                let bit = ((sample >> bit_index) & 1) as u8;
