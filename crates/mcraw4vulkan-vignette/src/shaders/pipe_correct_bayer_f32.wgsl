// Rust writes this exact 128-byte sequence as 32 little-endian words. This is
// the PIPE-only relative-linear f32 terminal; the DNG packed-U16 terminal uses
// a different shader and remains unchanged.
struct Params {
...
fn compact_gain_q_for_pixel(pixel_index: u32) -> U64Parts {
    if (params.spatial_mode == 0u) {
        var identity: U64Parts;
        identity.lo = 65536u;
        identity.hi = 0u;
        return identity;
    }
...
fn corrected_sample(pixel_index: u32, raw_sample: u32) -> f32 {
    let x = pixel_index % params.frame_width;
    let y = pixel_index / params.frame_width;
    let plane_index = ((y & 1u) * 2u) + (x & 1u);
    let black_q = black_q_for_plane(plane_index);
    let black_f = f32(black_q) / 65536.0;
    let gain_f = f32(final_gain_q16_for_pixel(pixel_index)) / 65536.0;
    let corrected_code_f = (f32(raw_sample) - black_f) * gain_f;
    return corrected_code_f / f32(params.corrected_white);
}
...
fn main(
    ...
    let group_index = workgroup_id.y * num_workgroups.x + workgroup_id.x;
    let total_workgroups = params.global_pixel_count / 256u
        + select(0u, 1u, (params.global_pixel_count % 256u) != 0u);
    if (group_index >= total_workgroups) {
        return;
    }
    let pixel_index = group_index * 256u + local_invocation_index;
    if (pixel_index >= params.global_pixel_count) {
        return;
    }

    let packed_word_index = pixel_index / 2u;
    let word = input_words[packed_word_index];
    var raw_sample = word & 0xffffu;
    if ((pixel_index & 1u) == 1u) {
        raw_sample = (word >> 16u) & 0xffffu;
    }
    output_samples[pixel_index] = corrected_sample(pixel_index, raw_sample);
}
