...
#[cfg(test)]
mod tests {
    use mcraw4vulkan_mcrawcontainer::LensShadingMap;

    use super::*;

    fn source_metadata() -> ContainerMetadata {
    ...
    fn frame_metadata() -> FrameMetadata {
        FrameMetadata::parse(
            r#"{
                "width": 8,
                "height": 6,
    ...
    #[test]
    fn motioncam_pipe_facts_use_dynamic_source_levels_and_requested_spatial_mode() {
        let container = source_metadata();
        let mut frame = frame_metadata();
        frame.dynamic_black_level = Some([60.25, 61.5, 62.75, 63.0]);
        frame.dynamic_white_level = Some(1023.0);
        frame.lens_shading_map = Some(
            LensShadingMap::new(
                2,
                2,
                [1.0, 1.125, 1.25, 1.5]
        ...
        let facts = motioncam_pipe_f32_bayer_facts(
            &container,
            &frame,
            BayerPattern::Rggb,
            PipeF32BayerCorrectionMode::MotionCamSpatial,
        )
        .expect("spatial correction facts");

        assert_eq!(facts.frame_dimensions, frame.dimensions);
        assert_eq!(facts.bayer_pattern, BayerPattern::Rggb);
        assert_eq!(facts.input_black_level, [60.25, 61.5, 62.75, 63.0]);
        assert_eq!(facts.output_white_level, 1023);
        assert!(facts.lens_shading_map.is_some());
    }

    #[test]
    fn identity_spatial_gain_retains_source_levels_without_requiring_a_map() {
        let container = source_metadata();
        let frame = frame_metadata();
        let facts = motioncam_pipe_f32_bayer_facts(
            &container,
            &frame,
            BayerPattern::Rggb,
        ...
        .expect("identity correction facts");

        assert_eq!(facts.input_black_level, [64.0, 65.0, 66.0, 67.0]);
        assert_eq!(facts.output_white_level, 4095);
        assert!(facts.lens_shading_map.is_none());
    }

    #[test]
    fn motioncam_spatial_requires_a_present_map() {
        let error = motioncam_pipe_f32_bayer_facts(
            &source_metadata(),
            &frame_metadata(),
            BayerPattern::Rggb,
            PipeF32BayerCorrectionMode::MotionCamSpatial,
        )
        .expect_err("missing spatial map must fail");

        assert_eq!(
            error,
            VignetteCorrectionError::MotionCamSpatialMissingLensShadingMap
        );
    }
}
