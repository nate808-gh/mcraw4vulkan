...
mod tests {
    ...
    #[test]
    fn f32_power2_rational_round_trips_motioncam_constants() {
        for value in [MOTIONCAM_PIXEL_GAIN_LINEAR_STRENGTH, 4.0, 4.267_708] {
            let (numerator, shift) = f32_power2_rational(value).expect("positive finite value");
            let reconstructed = numerator as f64 / (2_u64.pow(shift) as f64);
            assert_eq!(reconstructed as f32, value);
        }
    }

    #[test]
    fn output_byte_contract_is_exact_for_required_shapes() {
        for (width, height, expected) in [
            ...
            (257, 1, 1_028),
            (1920, 1080, 8_294_400),
            (3840, 2160, 33_177_600),
            (4080, 3072, 50_135_040),
            (8192, 4608, 150_994_944),
        ] {
            assert_eq!(width * height * PIPE_F32_SAMPLE_BYTES, expected);
        }
    }

    #[test]
    fn resource_contract_reports_exact_limit_failures() {
        let permissive = PipeF32Limits {
            max_storage_buffer_binding_size: 1_000_000,
            max_buffer_size: 1_000_000,
            max_compute_workgroups_per_dimension: 100,
        };
        assert_eq!(
            validate_resource_contract(257, 516, 1_028, 8, permissive)
                .expect("257 samples use two workgroups"),
            PipeF32DispatchGrid {
                x: 2,
                y: 1,
                total: 2,
        ...
        let binding_error = validate_resource_contract(
            1,
            4,
            4,
            12,
        ...
        .expect_err("oversized compact map binding is rejected");
        assert_eq!(
            binding_error,
            PipeF32BayerError::StorageBindingTooLarge {
                buffer: "compact gain map",
                required_bytes: 12,
                max_binding_bytes: 8,
            }
        );

        let buffer_error = validate_resource_contract(
            4,
            8,
            16,
            8,
        ...
        .expect_err("oversized allocation is rejected independently of binding size");
        assert_eq!(
            buffer_error,
            PipeF32BayerError::OutputBufferTooLarge {
                required_bytes: 16,
                max_buffer_bytes: 12,
            }
        );

        let dispatch_error = validate_resource_contract(
            257,
            516,
            1_028,
            8,
        ...
        .expect_err("too many workgroups are rejected before dispatch");
        assert_eq!(
            dispatch_error,
            PipeF32BayerError::DispatchTooLarge {
                required_workgroups: 2,
                max_workgroups: 1,
            }
        );

        for pixel_count in [0, usize::MAX] {
            let error = validate_resource_contract(
                pixel_count,
                4,
                4,
                4,
            ...
            .expect_err("zero or overflowing dispatch geometry is rejected");
            assert!(matches!(error, PipeF32BayerError::DispatchTooLarge { .. }));
        }
        let zero_limit_error = validate_resource_contract(
            1,
            4,
            4,
            4,
        ...
        .expect_err("a zero per-dimension dispatch limit is rejected");
        assert_eq!(
            zero_limit_error,
            PipeF32BayerError::DispatchTooLarge {
                required_workgroups: 1,
                max_workgroups: 0,
            }
        );
    }
    ...
    #[test]
    fn dispatch_grid_preserves_small_shape_and_flattens_8k_only_when_needed() {
        let limits = PipeF32Limits {
            max_storage_buffer_binding_size: 226_492_416,
            max_buffer_size: 268_435_456,
            max_compute_workgroups_per_dimension: 65_535,
        };
        let boundary = validate_resource_contract(
            65_535 * PIPE_CORRECT_BAYER_F32_WORKGROUP_SIZE as usize,
            4,
            4,
            4,
            limits,
        )
        .expect("one-dimensional boundary fits");
        assert_eq!(
            boundary,
            PipeF32DispatchGrid {
                x: 65_535,
        ...
        let first_two_dimensional = validate_resource_contract(
            65_535 * PIPE_CORRECT_BAYER_F32_WORKGROUP_SIZE as usize + 1,
            4,
            4,
            4,
            limits,
        )
        .expect("the first oversized one-dimensional job uses a 2D grid");
        assert_eq!(first_two_dimensional.x, 32_768);
        assert_eq!(first_two_dimensional.y, 2);
        assert_eq!(first_two_dimensional.total, 65_536);

        let eight_k = validate_resource_contract(8_192 * 4_608, 75_497_472, 150_994_944, 8, limits)
            .expect("8192x4608 Candidate-4 resources and 2D grid fit");
        assert_eq!(
            eight_k,
            PipeF32DispatchGrid {
                x: 49_152,
        ...
        assert!(u64::from(eight_k.x) * u64::from(eight_k.y) >= u64::from(eight_k.total));
    }

    #[test]
    fn shader_flattens_2d_workgroups_before_forming_the_pixel_index() {
        let source = PIPE_CORRECT_BAYER_F32_WGSL;
        let group = source.find("let group_index").expect("group flattening");
        let group_guard = source
            .find("group_index >= total_workgroups")
            .expect("group overflow/padding guard");
        let pixel = source.find("let pixel_index").expect("pixel index");
        let pixel_guard = source
            .find("pixel_index >= params.global_pixel_count")
            .expect("pixel padding guard");
        assert!(source.contains("@builtin(workgroup_id)"));
        assert!(source.contains("@builtin(num_workgroups)"));
        assert!(source.contains("@builtin(local_invocation_index)"));
        assert!(group < group_guard && group_guard < pixel && pixel < pixel_guard);
    }

    #[test]
    fn identity_mode_has_a_distinct_semantic_label() {
        assert_eq!(
            PipeF32BayerCorrectionMode::IdentitySpatialGain.label(),
            "identity-spatial-gain"
        );
        assert_ne!(
    ...
    #[test]
    fn correction_fingerprint_record_is_exact_and_versioned() {
        let dimensions = FrameDimensions {
            width: 4,
            height: 3,
        };
        let map = constant_map(2, 2, [1.0, 1.25, 1.5, 1.75]);
        let spatial_facts = fixed_facts(
            dimensions,
            BayerPattern::Rggb,
            Some(&map),
            [64.0, 65.0, 66.0, 67.0],
        ...
        let spatial = PipeF32BayerCorrectionFingerprint::from_fixed_facts(
            &spatial_facts,
            PipeF32BayerCorrectionMode::MotionCamSpatial,
        )
        .expect("spatial fingerprint resolves");
        let record = spatial.record_bytes();

        assert_eq!(record.len(), 88);
        assert_eq!(&record[..36], b"PipeF32BayerCorrectionFingerprintV1\0");
        assert_eq!(record[36], 1);
        assert_eq!(record[37], 1);
        for (index, word) in spatial
            .spatial
            .expect("spatial identity")
            .words()
            .into_iter()
            .enumerate()
        {
            let start = 38 + index * size_of::<u64>();
            assert_eq!(
                &record[start..start + size_of::<u64>()],
                &word.to_le_bytes()
            );
        }
        for (index, word) in spatial.conversion.words().into_iter().enumerate() {
            let start = 54 + index * size_of::<u64>();
            assert_eq!(
                &record[start..start + size_of::<u64>()],
                &word.to_le_bytes()
        ...
        for (index, black) in spatial.black_q.into_iter().enumerate() {
            let start = 70 + index * size_of::<u32>();
            assert_eq!(
                &record[start..start + size_of::<u32>()],
                &black.to_le_bytes()
            );
        }
        assert_eq!(&record[86..], &spatial.corrected_white.to_le_bytes());

        let identity_facts = fixed_facts(
            dimensions,
            BayerPattern::Rggb,
            None,
            [64.0, 65.0, 66.0, 67.0],
        ...
        let identity = PipeF32BayerCorrectionFingerprint::from_fixed_facts(
            &identity_facts,
            PipeF32BayerCorrectionMode::IdentitySpatialGain,
        )
        .expect("identity-spatial fingerprint resolves");
        let identity_record = identity.record_bytes();
        assert_eq!(identity_record[36], 0);
        assert_eq!(identity_record[37], 0);
        assert_eq!(&identity_record[38..54], &[0_u8; 16]);
    }

    #[test]
    fn identity_spatial_mode_rejects_dng_passthrough_correction_facts() {
        ...
        facts.mode = VignetteCorrectionMode::Disabled;
        assert_eq!(
            validate_correction_facts(&facts)
                .expect_err("DNG/pass-through Disabled cannot mean PIPE identity spatial gain"),
            PipeF32BayerError::UnsupportedInputMode {
    ...
    #[test]
    fn inconsistent_mutated_correction_facts_are_rejected() {
        let mut facts = fixed_facts(
            FrameDimensions {
                width: 2,
                height: 2,
        ...
        let expected = facts.pixel_domain;
        facts.pixel_domain.corrected_white_tag = 4094;
        assert_eq!(
            validate_correction_facts(&facts)
                .expect_err("a hand-mutated CW cannot bypass fact derivation"),
            PipeF32BayerError::InconsistentPixelDomain {
                expected,
                actual: facts.pixel_domain,
            }
        );

        facts.pixel_domain = expected;
        facts.input_black_level_q[2] += 1;
        assert_eq!(
            validate_correction_facts(&facts)
                .expect_err("a hand-mutated fixed black cannot reach the shader"),
            PipeF32BayerError::InconsistentFixedBlack {
                index: 2,
                expected: 64 * VIGNETTE_GAIN_SCALE,
                actual: 64 * VIGNETTE_GAIN_SCALE + 1,
            }
        );
    }
    ...
    fn fixed_facts<'a>(
        ...
        .expect("input facts validate");
        let fixed_map =
            map.map(|map| PreparedFixedLensShadingMap::from_typed_map(map).expect("map prepares"));
