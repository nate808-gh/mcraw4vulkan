...
mod tests {
    use super::{
        DEFAULT_WINDOW_HEIGHT, DEFAULT_WINDOW_WIDTH, Sdl2WgpuSurfaceConfig, Sdl2WgpuSurfaceError,
        WindowSize, choose_alpha_mode, choose_present_mode, choose_present_mode_with_preference,
        choose_surface_format,
    };

    #[test]
    fn default_config_uses_wayland_smoke_size_and_vulkan() {
        let config = Sdl2WgpuSurfaceConfig::default();
    ...
    #[test]
    fn window_builder_uses_config_size_without_startup_maximize_or_fullscreen() {
        let source = include_str!("lib.rs");
        let start = source.find("pub fn new").expect("new method");
        let end = source[start..].find("let instance").unwrap() + start;
        let body = &source[start..end];

        assert!(body.contains("video.window(&config.title, config.width, config.height)"));
        assert!(body.contains(".position_centered()"));
        assert!(body.contains(".resizable()"));
        assert!(body.contains(".allow_highdpi()"));
        assert!(!body.contains("maximized"));
        assert!(!body.contains("fullscreen"));
        assert!(!body.contains("set_size"));
    }

    #[test]
    fn config_rejects_zero_window_dimensions() {
        let error = Sdl2WgpuSurfaceConfig::new("bad", 0, 900)
            .validate()
            .expect_err("zero width must be rejected");

        assert!(matches!(
            error,
            Sdl2WgpuSurfaceError::InvalidWindowSize {
                width: 0,
                height: 900
            }
        ));
    ...
    #[test]
    fn window_size_reports_zero_drawable_and_scale() {
        let size = WindowSize::new(1600, 900, 0, 900);
    ...
    #[test]
    fn window_size_uses_drawable_to_window_scale() {
        let size = WindowSize::new(1600, 900, 3200, 1800);

        assert!(!size.is_zero());
        assert_eq!(size.scale_factor(), 2.0);
    }
    ...
    #[test]
    fn present_mode_prefers_vsync() {
        let mode = choose_present_mode(&[wgpu::PresentMode::Immediate, wgpu::PresentMode::Fifo]);

        assert_eq!(mode, Some(wgpu::PresentMode::Fifo));
    }

    #[test]
    fn present_mode_honors_supported_explicit_preference() {
        let modes = [wgpu::PresentMode::Fifo, wgpu::PresentMode::Immediate];

        assert_eq!(
            choose_present_mode_with_preference(&modes, Some(wgpu::PresentMode::Immediate)),
            Some(wgpu::PresentMode::Immediate)
        );
    }

    #[test]
    fn present_mode_preserves_auto_no_vsync_preference() {
        let modes = [
            wgpu::PresentMode::Fifo,
            wgpu::PresentMode::Immediate,
            wgpu::PresentMode::Mailbox,
        ...
        assert_eq!(
            choose_present_mode_with_preference(&modes, Some(wgpu::PresentMode::AutoNoVsync)),
            Some(wgpu::PresentMode::AutoNoVsync)
        );
    }

    #[test]
    fn present_mode_preserves_auto_vsync_preference() {
        let modes = [wgpu::PresentMode::Fifo, wgpu::PresentMode::Immediate];

        assert_eq!(
            choose_present_mode_with_preference(&modes, Some(wgpu::PresentMode::AutoVsync)),
            Some(wgpu::PresentMode::AutoVsync)
        );
    }

    #[test]
    fn present_mode_falls_back_for_unsupported_concrete_preference() {
        let modes = [wgpu::PresentMode::Fifo, wgpu::PresentMode::Immediate];

        assert_eq!(
            choose_present_mode_with_preference(&modes, Some(wgpu::PresentMode::Mailbox)),
    ...
    fn alpha_mode_prefers_opaque() {
        let mode = choose_alpha_mode(&[
            wgpu::CompositeAlphaMode::Inherit,
            wgpu::CompositeAlphaMode::Opaque,
        ]);

        assert_eq!(mode, Some(wgpu::CompositeAlphaMode::Opaque));
    }

    #[test]
    fn error_display_includes_invalid_size() {
        let error = Sdl2WgpuSurfaceError::InvalidWindowSize {
            width: 0,
            height: 0,
        };

        assert_eq!(error.to_string(), "invalid SDL2 window size 0x0");
    }

    #[test]
    fn maximize_window_is_safe_sdl_delegate_without_resize_or_platform_branch() {
        let source = include_str!("lib.rs");
        let start = source
            .find("pub fn maximize_window")
            .expect("maximize_window method");
        let body = &source[start..source[start..].find("pub fn adapter_info").unwrap() + start];

        assert!(body.contains("self.window.maximize();"));
        assert!(!body.contains("unsafe"));
        assert!(!body.contains("set_fullscreen"));
        assert!(!body.contains("reconfigure"));
        for forbidden in ["x11", "wayland", "gnome", "kde", "cosmic"] {
            assert!(!body.to_ascii_lowercase().contains(forbidden));
        }
    }

    #[test]
    fn set_window_size_is_safe_sdl_delegate_without_reconfigure_or_platform_branch() {
        let source = include_str!("lib.rs");
        let start = source
            .find("pub fn set_window_size")
            .expect("set_window_size method");
        let body = &source[start..source[start..].find("pub fn adapter_info").unwrap() + start];

        assert!(body.contains("self.window.restore();"));
        assert!(body.contains(".set_size(width, height)"));
        assert!(body.contains("WindowSize::from_window(&self.window)"));
        assert!(!body.contains("unsafe"));
        assert!(!body.contains("set_fullscreen"));
        assert!(!body.contains("maximize"));
        assert!(!body.contains("reconfigure"));
        for forbidden in ["x11", "wayland", "gnome", "kde", "cosmic"] {
            assert!(!body.to_ascii_lowercase().contains(forbidden));
        }
