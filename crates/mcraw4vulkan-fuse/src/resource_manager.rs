...
#[cfg(test)]
mod tests {
    use mcraw4vulkan_core::DecodeBackendChoice;

    use super::{
        AppResourceManager, ClipMountState, ClipPreviewState, DEFAULT_DNG_CACHE_FRAME_CAPACITY,
        DEFAULT_PREFETCH_FORWARD_FRAMES, RegisteredClipConfig,
    };

    #[test]
    fn registers_clip_and_returns_stable_record() {
        let manager = AppResourceManager::new();

        let clip_id = manager
            .register_clip(RegisteredClipConfig::new(
                "fixtures/example.mcraw",
                DecodeBackendChoice::Gpu,
            ))
        ...
        assert_eq!(record.clip_id, clip_id);
        assert_eq!(record.display_name, "example");
        assert_eq!(record.backend_choice, DecodeBackendChoice::Gpu);
        assert_eq!(
            record.max_cached_dng_frames,
            DEFAULT_DNG_CACHE_FRAME_CAPACITY
        );
        assert_eq!(
            record.prefetch_forward_frames,
            DEFAULT_PREFETCH_FORWARD_FRAMES
        );
        assert_eq!(record.mount_state, ClipMountState::Unmounted);
        assert_eq!(record.preview_state, ClipPreviewState::Inactive);
    }

    #[test]
    fn registered_clip_ids_are_sorted() {
        let manager = AppResourceManager::new();

        let first = manager
            .register_clip(RegisteredClipConfig::new(
                "fixtures/first.mcraw",
                DecodeBackendChoice::Gpu,
            ))
            .expect("register first");
        let second = manager
            .register_clip(RegisteredClipConfig::new(
                "fixtures/second.mcraw",
                DecodeBackendChoice::Cpu,
            ))
            .expect("register second");

        let ids = manager
            .registered_clip_ids()
            .expect("list IDs should not fail");

        assert_eq!(ids, vec![first, second]);
    }

    #[test]
    fn updates_lightweight_states() {
        let manager = AppResourceManager::new();
        ...
        manager
            .set_mount_state(clip_id, ClipMountState::Mounted)
            .expect("set mount state");
        manager
            .set_preview_state(clip_id, ClipPreviewState::Selected)
            .expect("set preview state");
        ...
        assert_eq!(record.mount_state, ClipMountState::Mounted);
        assert_eq!(record.preview_state, ClipPreviewState::Selected);
    }
}
