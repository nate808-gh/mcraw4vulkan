...
mod tests {
    ...
    fn dummy_frame(frame_index: usize, bytes: usize) -> Arc<CachedDngFrame> {
        Arc::new(CachedDngFrame {
            frame_index,
            bytes: Arc::from(vec![0; bytes].into_boxed_slice()),
            byte_len: u64::try_from(bytes).unwrap(),
            timings: DngGenerationTimings::default(),
        })
    }

    fn insert_dummy(state: &mut CacheState, frame_index: usize, bytes: usize) {
        let frame = dummy_frame(frame_index, bytes);
        state.total_cached_bytes = state.total_cached_bytes.saturating_add(frame.byte_len);
        state.entries.insert(frame_index, frame);
        touch_lru_key(&mut state.lru_order, frame_index);
    }

    #[test]
    fn mount_lifetime_resource_is_lazy_reused_and_dropped_once() {
        ...
        assert!(slot.is_none());
        assert_eq!(
            initialize_mount_lifetime_resource(&mut slot, || {
        ...
        drop(take_mount_lifetime_resource(&mut slot));
        assert!(slot.is_none());
    ...
    #[test]
    fn failed_mount_lifetime_construction_is_not_cached_and_can_retry() {
        let attempts = AtomicUsize::new(0);
        let drops = Arc::new(AtomicUsize::new(0));
        let mut slot = None;

        let failed = initialize_mount_lifetime_resource(&mut slot, || {
            attempts.fetch_add(1, Ordering::Relaxed);
            Err::<DropProbe, _>("construction failed")
        });
        assert_eq!(failed, Err("construction failed"));
        assert!(slot.is_none());

        assert_eq!(
            initialize_mount_lifetime_resource(&mut slot, || {
                attempts.fetch_add(1, Ordering::Relaxed);
                Ok::<_, &str>(DropProbe(drops.clone()))
            }),
            Ok(true)
        );
    ...
    #[test]
    fn independent_mount_lifetime_resources_are_not_shared() {
        ...
        assert_eq!(
            initialize_mount_lifetime_resource(&mut first, || {
                Ok::<_, ()>(DropProbe(first_drops.clone()))
            }),
            Ok(true)
        );
        assert_eq!(
            initialize_mount_lifetime_resource(&mut second, || {
                Ok::<_, ()>(DropProbe(second_drops.clone()))
            }),
            Ok(true)
        );

        drop(take_mount_lifetime_resource(&mut first));
        assert_eq!(first_drops.load(Ordering::Relaxed), 1);
        assert_eq!(second_drops.load(Ordering::Relaxed), 0);
        drop(take_mount_lifetime_resource(&mut second));
        assert_eq!(second_drops.load(Ordering::Relaxed), 1);
    }

    #[test]
    fn production_policy_selects_21_4_hot_window() {
        let policy = DngCachePolicy::production_default();
        assert_eq!(policy.hot_frame_capacity(), 21);
        assert_eq!(policy.hot_prefetch_target(), 4);
        assert_eq!(policy.hot_history_capacity(), 16);
        assert_eq!(policy.queue_capacity(), 6);
        assert_eq!(DEFAULT_DNG_HOT_BYTE_CAPACITY, 2 * 1024 * MIB);
        assert_eq!(policy.hot_byte_capacity(), DEFAULT_DNG_HOT_BYTE_CAPACITY);
        assert_eq!(policy.cold_frame_capacity(), 1);
        assert_eq!(policy.hot_grace(), Duration::from_millis(700));
    }

    #[test]
    fn production_hot_policy_keeps_nominal_4k_window_below_byte_ceiling() {
        let policy = DngCachePolicy::production_default();
        let mut state = CacheState::default();
        insert_dummy(&mut state, 1, 1);
        state.total_cached_bytes = u64::try_from(DEFAULT_DNG_HOT_FRAME_CAPACITY)
            .unwrap()
            .saturating_mul(NOMINAL_4K_DNG_FRAME_BYTES);
        assert!(state.total_cached_bytes < policy.hot_byte_capacity());

        let (evicted, _) = evict_to_limits_locked(
            &mut state,
            policy.hot_frame_capacity(),
            policy.hot_byte_capacity(),
            None,
        );
    ...
    #[test]
    fn one_or_two_requests_do_not_promote() {
        let mut history = DemandHistory::default();
        assert_eq!(history.observe(10), DemandObservation::RandomOrIsolated);
        assert_eq!(
            history.observe(11),
            DemandObservation::Adjacent {
                direction: DngReadDirection::Forward,
                run_len: 2
            }
        );
    }
    ...
    #[test]
    fn three_adjacent_forward_requests_promote() {
        let mut history = DemandHistory::default();
        let _ = history.observe(10);
        let _ = history.observe(11);
        assert_eq!(
            history.observe(12),
            DemandObservation::Adjacent {
                direction: DngReadDirection::Forward,
    ...
    #[test]
    fn three_adjacent_reverse_requests_promote() {
        let mut history = DemandHistory::default();
        let _ = history.observe(50);
        let _ = history.observe(49);
        assert_eq!(
            history.observe(48),
            DemandObservation::Adjacent {
                direction: DngReadDirection::Backward,
                run_len: 3
            }
        );
    }
    ...
    #[test]
    fn prefetch_submission_keeps_adjacent_targets_in_one_batch() {
        let target_frames = [21, 22, 23, 24];
        let mut submissions = 0;

        let generated = submit_prefetch_batch(&target_frames, |submitted| {
            submissions += 1;
            assert_eq!(submitted, target_frames);
            Ok(submitted.to_vec())
        })
        .expect("batch submission succeeds");

        assert_eq!(submissions, 1);
        assert_eq!(generated, target_frames);
    }

    #[test]
    fn nonadjacent_thumbnail_sequence_resets_history() {
        let mut history = DemandHistory::default();
        let _ = history.observe(0);
        assert_eq!(history.observe(30), DemandObservation::RandomOrIsolated);
        assert_eq!(history.adjacent_run_len, 1);
        assert_eq!(history.direction, None);
    }

    #[test]
    fn direction_helpers_clip_to_frame_bounds() {
        assert_eq!(DngReadDirection::Forward.next_frame(0, 2), Some(1));
        assert_eq!(DngReadDirection::Forward.next_frame(1, 2), None);
        assert_eq!(DngReadDirection::Backward.next_frame(1, 2), Some(0));
        assert_eq!(DngReadDirection::Backward.next_frame(0, 2), None);
    }

    #[test]
    fn lru_count_eviction_preserves_current_frame() {
        let mut state = CacheState::default();
        insert_dummy(&mut state, 1, 10);
        insert_dummy(&mut state, 2, 10);
        insert_dummy(&mut state, 3, 10);

        let (evicted, _) = evict_to_limits_locked(&mut state, 2, 100, Some(1));
    ...
    #[test]
    fn byte_eviction_uses_lru_and_preserves_current_oversize() {
        let mut state = CacheState::default();
        insert_dummy(&mut state, 1, 30);
        insert_dummy(&mut state, 2, 30);
        insert_dummy(&mut state, 3, 90);

        let (evicted, bytes) = evict_to_limits_locked(&mut state, 3, 80, Some(3));

        assert_eq!(evicted, 2);
        assert_eq!(bytes, 60);
        assert_eq!(state.entries.len(), 1);
        assert_eq!(state.total_cached_bytes, 90);
        assert!(state.entries.contains_key(&3));
    }

    #[test]
    fn cold_shrink_keeps_only_current_frame() {
        let mut state = CacheState::default();
        insert_dummy(&mut state, 1, 10);
        insert_dummy(&mut state, 2, 10);
        insert_dummy(&mut state, 3, 10);
        ...
        let policy = DngCachePolicy::production_default();
        let (evicted, _) = shrink_to_cold_locked(&mut state, policy, Some(2));
    ...
    #[test]
    fn demotion_waits_for_grace_and_active_work() {
        let mut state = CacheState::default();
        let policy = DngCachePolicy::production_default();
        let now = Instant::now();
        state.temperature = DngCacheTemperature::Hot {
            direction: DngReadDirection::Forward,
            last_demand: now - policy.hot_grace(),
        };
        state.active_demands = 1;
        assert!(!should_demote_locked(&state, policy, now));
