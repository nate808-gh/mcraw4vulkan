...
#[cfg(test)]
mod tests {
    use super::{GpuCanonicalDngBatchRoute, route_for_gpu_canonical_dng_layout};
    use mcraw4vulkan_core::FramePayloadLayout;
    use mcraw4vulkan_mcrawcontainer::FrameMetadata;

    fn route_for_layouts(layouts: &[FramePayloadLayout]) -> GpuCanonicalDngBatchRoute {
        layouts.iter().copied().fold(
            GpuCanonicalDngBatchRoute::InFlightRawcodecRing,
            route_for_gpu_canonical_dng_layout,
        )
    }

    #[test]
    fn dng_gpu_batch_route_keeps_type7_on_in_flight_rawcodec_ring() {
        let layout = FramePayloadLayout::CompressedRawcodecType7;

        assert!(layout.supports_native_gpu_decode());
        assert!(layout.uses_compressed_rawcodec_work_plan());
        assert_eq!(
            route_for_layouts(&[layout]),
            GpuCanonicalDngBatchRoute::InFlightRawcodecRing
        );
    }

    #[test]
    fn dng_gpu_batch_route_accepts_type6_through_serial_canonical_path() {
        let layout = FramePayloadLayout::BinnedRaw16Type6 { row_stride: 3840 };

        assert!(layout.supports_native_gpu_decode());
        assert!(!layout.uses_compressed_rawcodec_work_plan());
        assert_eq!(layout.unsupported_gpu_decode_message(), None);
        assert_eq!(
            route_for_layouts(&[layout]),
            GpuCanonicalDngBatchRoute::SerialCanonical
        );
    ...
    #[test]
    fn dng_gpu_batch_route_uses_serial_path_for_mixed_supported_layouts() {
        let type7 = FramePayloadLayout::CompressedRawcodecType7;
        let type6 = FramePayloadLayout::BinnedRaw16Type6 { row_stride: 3840 };

        assert_eq!(
            route_for_layouts(&[type7, type6]),
            GpuCanonicalDngBatchRoute::SerialCanonical
        );
    }

    #[test]
    fn dng_gpu_batch_route_still_rejects_unknown_metadata_before_decode() {
        let metadata = FrameMetadata::parse(
            r#"{
                "width": 1920,
                "height": 1080,
                "rowStride": 3840,
        ...
        let error = metadata
            .payload_layout()
            .expect_err("unknown compression type must remain unsupported");
        let message = error.to_string();
        assert!(message.contains("unsupported MotionCam payload layout"));
