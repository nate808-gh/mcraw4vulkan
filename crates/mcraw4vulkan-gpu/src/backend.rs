...
mod tests {
    ...
    #[test]
    fn compact_work_item_is_sixteen_bytes() {
        assert_eq!(std::mem::size_of::<McrawVulkanPackedBlockWorkItem>(), 16);
        assert_eq!(work_item_byte_len(3).unwrap(), 48);
    }

    #[test]
    fn work_plan_allocation_stats_report_block_shape() {
        let mut blocks = Vec::with_capacity(8);
        blocks.extend([sample_work_item(), sample_work_item(), sample_work_item()]);
        let work_plan = McrawVulkanWorkPlan {
        ...
        let stats = GpuWorkPlanAllocationStats::from_work_plan(&work_plan);

        assert_eq!(stats.blocks_len, 3);
        assert_eq!(stats.blocks_capacity, 8);
        assert_eq!(stats.work_items_len, 3);
        assert_eq!(stats.work_items_capacity, 8);
        assert_eq!(stats.work_items_bytes, 48);
        assert!(!stats.reused_scratch);
    }

    #[test]
    fn pack_gpu_work_item_produces_expected_words() {
        let item = McrawVulkanBlockWorkItem {
            payload_offset: 0x1234_5678,
            payload_len: 0x34,
            raw_encoding: 0x12,
            reference_value: 0xbeef,
            macroblock_x: 0x4567,
            macroblock_y: 0x89ab,
            lane_index: 0xcd,
            reserved: 0,
        ...
        let packed = pack_gpu_work_item(&item).unwrap();

        assert_eq!(packed.word0, 0x1234_5678);
        assert_eq!(packed.word1, 0x3412_beef);
        assert_eq!(packed.word2, 0x89ab_4567);
        assert_eq!(packed.word3, 0x0000_00cd);
    }

    #[test]
    fn rust_unpacking_matches_wgsl_masks() {
        let item = McrawVulkanBlockWorkItem {
            payload_offset: 0x0102_0304,
            payload_len: 128,
            raw_encoding: 16,
            reference_value: 0xabcd,
            macroblock_x: 8192,
            macroblock_y: 4321,
            lane_index: 3,
            reserved: 0,
        ...
        let payload_offset = packed.word0;
        let reference_value = packed.word1 & 0xffff;
        let raw_encoding = (packed.word1 >> 16) & 0xff;
        let payload_len = (packed.word1 >> 24) & 0xff;
        let macroblock_x = packed.word2 & 0xffff;
        let macroblock_y = (packed.word2 >> 16) & 0xffff;
        let lane_index = packed.word3 & 0xff;

        assert_eq!(payload_offset, item.payload_offset);
        assert_eq!(reference_value, item.reference_value);
        assert_eq!(raw_encoding, item.raw_encoding);
        assert_eq!(payload_len, item.payload_len);
        assert_eq!(macroblock_x, item.macroblock_x);
        assert_eq!(macroblock_y, item.macroblock_y);
        assert_eq!(lane_index, item.lane_index);
    }

    #[test]
    fn pack_gpu_work_item_rejects_out_of_range_fields() {
        let base = sample_work_item();
        let cases = [
            (
                "reference_value",
                McrawVulkanBlockWorkItem {
                    reference_value: u32::from(u16::MAX) + 1,
                    ..base
                },
            ),
            (
                "raw_encoding",
                McrawVulkanBlockWorkItem {
                    raw_encoding: u32::from(u8::MAX) + 1,
                    ..base
                },
            ),
            (
            ...
            (
                "macroblock_x",
                McrawVulkanBlockWorkItem {
                    macroblock_x: u32::from(u16::MAX) + 1,
                    ..base
                },
            ),
            (
                "macroblock_y",
                McrawVulkanBlockWorkItem {
                    macroblock_y: u32::from(u16::MAX) + 1,
                    ..base
                },
            ),
            (
        ...
        for (field, item) in cases {
            let error = pack_gpu_work_item(&item).unwrap_err().to_string();
            assert!(
                error.contains(field),
                "error for {field} should name field, got: {error}"
            );
        }
    }

    #[test]
    fn pack_gpu_work_item_rejects_nonzero_reserved_field() {
        let item = McrawVulkanBlockWorkItem {
            reserved: 1,
            ..sample_work_item()
        };
        ...
        assert!(error.contains("must be zero"));
    }

    #[test]
    fn prepare_upload_staging_reports_compact_work_item_bytes() {
        let mut buffers = ReusableGpuBuffers::default();
        let item = sample_work_item();

        let counts = prepare_upload_staging(&[1, 2, 3], &[item, item], &mut buffers).unwrap();

        assert_eq!(counts.raw_payload_byte_len, 4);
        assert_eq!(counts.work_item_byte_len, 32);
        assert_eq!(buffers.work_item_upload_scratch.len(), 32);
        assert!(!counts.work_item_buffer_reused);
        assert_eq!(counts.work_item_buffer_reallocs, 1);
    }

    #[test]
    fn legacy_raw16_work_plan_records_two_blocks_per_chunk() {
        let payload = [0x01, 0x23, 0x04, 0x56];
        let dimensions = FrameDimensions {
            width: 2,
            height: 1,
        };

        let plan = build_legacy_raw16_gpu_work_plan(&payload, dimensions, 4).unwrap();
    ...
    #[test]
    fn legacy_raw16_prepared_plan_reuses_caller_owned_descriptor_storage() {
        let payload = [0x01, 0x23, 0x04, 0x56];
        let dimensions = FrameDimensions {
            width: 2,
            height: 1,
        ...
        let first = scratch.prepare_type6(&payload, dimensions, 4).unwrap();
        assert_eq!(first.block_count(), 2);
        assert_eq!(first.scratch_grow_count(), 1);
        let first_capacity = first.block_capacity();

        let second = scratch.prepare_type6(&payload, dimensions, 4).unwrap();
        assert_eq!(second.block_count(), 2);
        assert_eq!(second.block_capacity(), first_capacity);
        assert!(second.reused_scratch());
        assert_eq!(second.scratch_grow_count(), 0);
    }

    #[test]
    fn legacy_raw16_work_plan_rejects_short_payload() {
        let dimensions = FrameDimensions {
            width: 2,
            height: 1,
        };

        let error = build_legacy_raw16_gpu_work_plan(&[0x10, 0x00, 0xff], dimensions, 4)
            .unwrap_err()
            .to_string();

        assert!(
            error.contains("legacy raw16 payload is too short"),
            "{error}"
        );
    }

    #[test]
    fn legacy_raw16_work_plan_rejects_short_row_stride() {
        let dimensions = FrameDimensions {
            width: 4,
            height: 1,
        };

        let error = build_legacy_raw16_gpu_work_plan(&[], dimensions, 7)
            .unwrap_err()
            .to_string();

        assert!(error.contains("raw16 rowStride is too short"), "{error}");
        assert!(error.contains("minimum=8"), "{error}");
    }

    #[test]
    fn legacy_raw16_expected_invocations_use_legacy_workgroups() {
        assert_eq!(legacy_raw16_expected_invocations(1, 65_535).unwrap(), 128);
        assert_eq!(legacy_raw16_expected_invocations(9, 65_535).unwrap(), 256);
    }

    #[test]
    fn legacy_raw16_shader_source_is_available() {
        assert!(MCRAW_DECODE_LEGACY_RAW16_PACKED_U16_WGSL.contains("decode_legacy_raw16_delta"));
        assert!(MCRAW_DECODE_LEGACY_RAW16_PACKED_U16_WGSL.contains("@workgroup_size(128)"));
    }

    #[test]
    fn reusable_work_item_buffer_reports_reuse() {
        let mut buffers = ReusableGpuBuffers::default();
        let item = sample_work_item();

        let first = prepare_upload_staging(&[1, 2, 3, 4], &[item, item], &mut buffers).unwrap();
        let second = prepare_upload_staging(&[5, 6, 7, 8], &[item, item], &mut buffers).unwrap();

        assert_eq!(first.work_item_byte_len, second.work_item_byte_len);
        assert!(!first.work_item_buffer_reused);
        assert_eq!(first.work_item_buffer_reallocs, 1);
        assert!(second.work_item_buffer_reused);
        assert_eq!(second.work_item_buffer_reallocs, 0);
        assert_eq!(
            second.work_item_buffer_capacity,
            u64::try_from(buffers.work_item_upload_scratch.capacity()).unwrap()
        );
    }

    #[test]
    fn work_item_slice_writer_matches_reusable_vec_serializer() {
        let items = [
            sample_work_item(),
            McrawVulkanBlockWorkItem {
                payload_offset: 5,
        ...
        work_items_to_le_bytes_into(&items, &mut serialized).unwrap();

        let mut direct = vec![0u8; work_item_byte_len(items.len()).unwrap()];
        write_work_items_to_slice(&items, &mut direct).unwrap();

        assert_eq!(serialized, direct);
    }

    #[test]
    fn gpu_decode_config_default_disables_timestamps() {
        let config = GpuDecodeConfig::default();

        assert_eq!(config.backend_preference, GpuBackendPreference::Auto);
        assert!(!config.enable_gpu_timestamps);
    }

    fn synthetic_adapter_limits(storage_bytes: u32, buffer_bytes: u64) -> wgpu::Limits {
        wgpu::Limits {
            max_texture_dimension_1d: 16_384,
            max_texture_dimension_2d: 16_384,
            max_storage_buffer_binding_size: storage_bytes,
            max_buffer_size: buffer_bytes,
            ..wgpu::Limits::downlevel_defaults()
        }
    }

    fn direct_yuv_requirement(
        dimensions: FrameDimensions,
        storage_bytes: u64,
        buffer_bytes: u64,
    ) -> GpuDecodeRequiredLimits {
        GpuDecodeRequiredLimits::new(
            dimensions,
            "direct YUV12 output",
            storage_bytes,
    ...
    #[test]
    fn required_limits_ordinary_constructor_remains_downlevel_plus_adapter_resolution() {
        let adapter = synthetic_adapter_limits(512 << 20, 1 << 30);
        let requested = requested_device_limits(&adapter, None).expect("default limits");
        let expected = wgpu::Limits::downlevel_defaults().using_resolution(adapter.clone());
    ...
    #[test]
    fn required_limits_four_k_on_128_mib_adapter_keeps_downlevel_buffer_limits() {
        let adapter = synthetic_adapter_limits(128 << 20, 256 << 20);
        for (width, height, storage_bytes) in
            [(3_840, 2_160, 49_766_400), (4_080, 3_072, 75_202_560)]
        {
            let requirement = direct_yuv_requirement(
                FrameDimensions { width, height },
                storage_bytes,
                storage_bytes + 16,
            );
            let requested =
                requested_device_limits(&adapter, Some(&requirement)).expect("4K limits fit");
    ...
    #[test]
    fn required_limits_eight_k_requests_only_its_exact_extra_storage_binding() {
        let adapter = synthetic_adapter_limits(512 << 20, 1 << 30);
        let requirement = direct_yuv_requirement(
            FrameDimensions {
                width: 8_192,
        ...
        let requested =
            requested_device_limits(&adapter, Some(&requirement)).expect("8K limits fit");

        assert_eq!(requested.max_storage_buffer_binding_size, 226_492_416);
        assert_eq!(requested.max_buffer_size, 256 << 20);
        assert_eq!(requested.max_texture_dimension_2d, 16_384);
        assert_eq!(requested.max_compute_workgroups_per_dimension, 65_535);
        assert_eq!(requested.max_storage_buffers_per_shader_stage, 4);
        assert!(requested.check_limits(&adapter));
    }

    #[test]
    fn required_limits_storage_binding_also_raises_max_buffer_size_when_needed() {
        let adapter = synthetic_adapter_limits(512 << 20, 1 << 30);
        let requirement = direct_yuv_requirement(
            FrameDimensions {
                width: 9_000,
        ...
        let requested = requested_device_limits(&adapter, Some(&requirement))
            .expect("storage allocation fits adapter");

        assert_eq!(requested.max_storage_buffer_binding_size, 300 << 20);
        assert_eq!(requested.max_buffer_size, 300 << 20);
        assert!(requested.check_limits(&adapter));
    }

    #[test]
    fn required_limits_observed_probe_adapters_admit_the_exact_eight_k_request() {
        for adapter in [
            synthetic_adapter_limits(2_147_483_647, u64::MAX),
            synthetic_adapter_limits(2_147_483_647, 2_147_483_647),
        ] {
            let requirement = direct_yuv_requirement(
                FrameDimensions {
                    width: 8_192,
            ...
            let requested = requested_device_limits(&adapter, Some(&requirement))
                .expect("observed RTX/AMD wgpu limits admit 8K");
            assert_eq!(requested.max_storage_buffer_binding_size, 226_492_416);
            assert_eq!(requested.max_buffer_size, 256 << 20);
            assert!(requested.check_limits(&adapter));
        }
    ...
    #[test]
    fn required_limits_failure_reports_resource_dimensions_and_limit_provenance() {
        let dimensions = FrameDimensions {
            width: 8_192,
            height: 4_608,
        };
        let requirement = direct_yuv_requirement(dimensions, 226_492_416, 226_492_432);
        let storage_error = requested_device_limits(
            &synthetic_adapter_limits(128 << 20, 256 << 20),
            Some(&requirement),
        )
        .expect_err("128 MiB storage binding cannot admit the 8K job")
        .to_string();
        for expected in [
            "direct YUV12 output",
            "8192x4608",
            "max_storage_buffer_binding_size=226492416",
            "adapter-supported=134217728",
            "logical-device-requested=226492416",
        ] {
            assert!(storage_error.contains(expected), "{storage_error}");
        }

        let buffer_error = requested_device_limits(
            &synthetic_adapter_limits(512 << 20, 200 << 20),
            Some(&requirement),
        )
        .expect_err("small max buffer cannot admit the readback")
        .to_string();
        for expected in [
            "direct YUV12 output/status readback",
            "8192x4608",
            "max_buffer_size=226492432",
            "adapter-supported=209715200",
            "logical-device-requested=268435456",
        ] {
            assert!(buffer_error.contains(expected), "{buffer_error}");
        }
    }

    #[test]
    fn required_limits_invalid_or_unrepresentable_values_fail_before_device_creation() {
        let adapter = synthetic_adapter_limits(u32::MAX, u64::MAX);
        for (requirement, expected) in [
            (
                direct_yuv_requirement(
                    FrameDimensions {
                        width: 0,
                        height: 4_608,
                    },
            ...
            (
                ...
                "requires max_storage_buffer_binding_size=4294967296",
            ),
        ] {
            let error = requested_device_limits(&adapter, Some(&requirement))
    ...
    #[test]
    fn packed_u16_word_count_rounds_up() {
        assert_eq!(packed_u16_word_count(0), 0);
        assert_eq!(packed_u16_word_count(1), 1);
        assert_eq!(packed_u16_word_count(2), 1);
        assert_eq!(packed_u16_word_count(3), 2);
    }

    #[test]
    fn packed_u16_helpers_pack_low_then_high() {
        assert_eq!(pack_u16_pair(0x1234, 0xabcd), 0xabcd_1234);
        assert_eq!(pack_u16_last(0x2222), 0x0000_2222);
    }

    #[test]
    fn timestamp_delta_uses_picosecond_period() {
        let duration = timestamp_delta_duration(10, 16, 1_500).unwrap();

        assert_eq!(duration, Duration::from_nanos(9));
    }

    #[test]
    fn timestamp_query_indices_are_unique_and_counted() {
        let mut indices = vec![
            GpuTimestampQuery::TotalStart as u32,
            GpuTimestampQuery::OutputClearStart as u32,
            GpuTimestampQuery::OutputClearEnd as u32,
            GpuTimestampQuery::DecodeDispatchStart as u32,
            GpuTimestampQuery::DecodeDispatchEnd as u32,
            GpuTimestampQuery::VignetteDispatchStart as u32,
            GpuTimestampQuery::VignetteDispatchEnd as u32,
            GpuTimestampQuery::CopyToReadbackStart as u32,
            GpuTimestampQuery::CopyToReadbackEnd as u32,
            GpuTimestampQuery::TotalEnd as u32,
        ];
        indices.sort();
        indices.dedup();

        assert_eq!(indices.len(), GPU_TIMESTAMP_QUERY_COUNT_USIZE);
        assert_eq!(indices[0], 0);
        assert_eq!(indices[indices.len() - 1], GPU_TIMESTAMP_QUERY_COUNT - 1);
    }

    #[test]
    fn submitted_timestamp_profile_preserves_stage_mask() {
        let support = GpuTimestampSupport {
            requested: true,
            supported: true,
            enabled: true,
            period_ps: 1_000,
        };
        let submitted = GpuSubmittedTimestampProfile::from_stage_mask(
            support,
            GpuTimestampStageMask {
                output_clear: true,
                vignette: false,
        ...
        assert_eq!(submitted.support, support);
        assert!(submitted.output_clear_stage);
        assert!(!submitted.vignette_stage);
        assert!(submitted.preview_render_stage);
        assert_eq!(
            submitted.stage_mask().output_clear,
            submitted.output_clear_stage
        );
        assert_eq!(submitted.stage_mask().vignette, submitted.vignette_stage);
    }

    #[test]
    fn timestamp_profile_marks_unavailable_when_disabled() {
        let support = GpuTimestampSupport {
            requested: true,
            supported: false,
            enabled: false,
        ...
        let profile = GpuTimestampProfile::unavailable(support);

        assert!(profile.requested);
        assert!(!profile.supported);
        assert!(!profile.enabled);
        assert!(!profile.samples_valid);
        assert_eq!(profile.decode_dispatch, None);
    }

    fn sample_work_item() -> McrawVulkanBlockWorkItem {
        McrawVulkanBlockWorkItem {
            payload_offset: 1024,
            payload_len: 80,
            raw_encoding: 10,
