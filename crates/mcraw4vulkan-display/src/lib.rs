...
mod tests {
    ...
    #[test]
    fn prefers_bgra_srgb_surface_format() {
    ...
    #[test]
    fn falls_back_to_bgra_unorm_when_srgb_unavailable() {
        let selected = choose_surface_format(&[
            wgpu::TextureFormat::Rgba8Unorm,
            wgpu::TextureFormat::Bgra8Unorm,
        ]);
    ...
    #[test]
    fn present_mode_uses_safe_fallback_for_unsupported_explicit_modes() {
        let selected = choose_present_mode(&[wgpu::PresentMode::Fifo], DisplayPresentMode::Mailbox);

        assert_eq!(selected, wgpu::PresentMode::Fifo);
    }

    #[test]
    fn auto_no_vsync_is_selected_directly_for_backend_fallback() {
        let selected =
            choose_present_mode(&[wgpu::PresentMode::Fifo], DisplayPresentMode::AutoNoVsync);

        assert_eq!(selected, wgpu::PresentMode::AutoNoVsync);
    }

    #[test]
    fn display_defaults_use_vsync_fifo_policy() {
        assert_eq!(DisplayPresentMode::default(), DisplayPresentMode::Fifo);
        assert_eq!(
            DisplayWindowPolicy::default().present_policy.present_mode(),
            DisplayPresentMode::Fifo
        );
    }

    #[test]
    fn default_window_policy_is_1920_by_1080() {
        let policy = DisplayWindowPolicy::default();

        assert_eq!(policy.default_window_width, 1920);
        assert_eq!(policy.default_window_height, 1080);
        assert!(policy.preserve_aspect);
        assert!(policy.fullscreen_uses_current_monitor);
    }

    #[test]
    fn full_present_target_clears_from_surface_origin() {
        let size = DisplaySize {
            width: 1280,
            height: 720,
        };
        let target = DisplayPresentTarget::full(size);

        assert_eq!(target.origin_x, 0);
        assert_eq!(target.origin_y, 0);
        assert_eq!(target.size, size);
        assert_eq!(target.load_op, DisplayTargetLoadOp::Clear);
    }

    #[test]
    fn embedded_present_target_loads_existing_gui_frame() {
        let target = DisplayPresentTarget::embedded(24, 48, 720, 540).expect("valid target");

        assert_eq!(target.origin_x, 24);
        assert_eq!(target.origin_y, 48);
        assert_eq!(
            target.size,
            DisplaySize {
                width: 720,
        ...
        assert_eq!(target.load_op, DisplayTargetLoadOp::Load);
    }

    #[test]
    fn embedded_present_target_rejects_zero_size() {
        assert!(DisplayPresentTarget::embedded(0, 0, 0, 540).is_err());
        assert!(DisplayPresentTarget::embedded(0, 0, 720, 0).is_err());
    }

    #[test]
    fn fps_overlay_formats_two_decimals_and_units() {
        assert_eq!(
            fps_overlay_text_from_fps(24.0).as_deref(),
            Some("24.00 fps")
        );
        assert_eq!(
            fps_overlay_text_from_fps(29.97).as_deref(),
            Some("29.97 fps")
        );
        assert_eq!(
            fps_overlay_text_from_fps(61.0).as_deref(),
            Some("61.00 fps")
        );
        assert_eq!(
            fps_overlay_text_from_fps(138.10).as_deref(),
            Some("138.10 fps")
        );
    }

    #[test]
    fn fps_overlay_rejects_invalid_values_and_clamps_range() {
        assert!(DisplayFpsOverlay::from_fps(f64::NAN).is_none());
        assert!(DisplayFpsOverlay::from_fps(-1.0).is_none());
        assert_eq!(
            DisplayFpsOverlay::from_fps(29.97)
                .expect("valid fps")
                .fps_x100,
            2997
        );
        assert_eq!(
            DisplayFpsOverlay::from_fps(12_000.0)
                .expect("clamped fps")
                .fps_x100,
            DISPLAY_FPS_OVERLAY_MAX_X100
        );
    }

    #[test]
    fn fps_overlay_required_glyphs_are_supported() {
        for glyph in DISPLAY_FPS_OVERLAY_REQUIRED_GLYPHS.chars() {
            assert!(fps_overlay_supports_glyph(glyph), "missing glyph {glyph:?}");
        }
        assert!(DISPLAY_PRESENT_WGSL.contains("decimal_point_glyph"));
        assert!(DISPLAY_PRESENT_WGSL.contains("glyph_f"));
        assert!(DISPLAY_PRESENT_WGSL.contains("glyph_p"));
        assert!(DISPLAY_PRESENT_WGSL.contains("glyph_x"));
        assert!(DISPLAY_PRESENT_WGSL.contains("resolution_overlay_char"));
        assert!(DISPLAY_PRESENT_WGSL.contains("fps_overlay_char"));
    }

    #[test]
    fn fps_overlay_no_longer_formats_bare_numbers() {
        let text = fps_overlay_text_from_fps(30.0).expect("valid overlay text");

        assert_eq!(text, "30.00 fps");
        assert_ne!(text, "30");
        assert_ne!(text, "30.0");
    }

    #[test]
    fn fps_overlay_formats_source_resolution_above_fps_when_source_size_is_set() {
        let text = DisplayFpsOverlay::from_fps(30.0)
            .expect("valid fps")
            .with_source_size(DisplaySize {
                width: 3840,
                height: 2160,
            })
            .text();

        assert_eq!(text, "3840 x 2160\n\n30.00 fps");
    }

    #[test]
    fn fps_meter_reports_24fps_over_deterministic_interval() {
        let start = Instant::now();
        let mut meter =
            DisplayFpsMeter::with_min_sample_duration(start, Duration::from_millis(250));

        let fps = meter
            .record_presented_frames(24, start + Duration::from_secs(1))
            .expect("measured fps");

        assert!((fps - 24.0).abs() < 0.000001);
        assert_eq!(fps_overlay_text_from_fps(fps).as_deref(), Some("24.00 fps"));
    }

    #[test]
    fn fps_meter_ignores_cached_present_without_new_source_frame() {
        let start = Instant::now();
        let mut meter =
            DisplayFpsMeter::with_min_sample_duration(start, Duration::from_millis(250));

        assert_eq!(
            meter.record_presented_frames(0, start + Duration::from_secs(1)),
            None
        );

        let fps = meter
            .record_presented_frames(1, start + Duration::from_secs(1))
            .expect("measured fps");
        assert!((fps - 1.0).abs() < 0.000001);
    }

    #[test]
    fn fps_meter_reports_2997fps_over_deterministic_interval() {
        let start = Instant::now();
        let mut meter =
            DisplayFpsMeter::with_min_sample_duration(start, Duration::from_millis(250));

        let fps = meter
            .record_presented_frames(2997, start + Duration::from_secs(100))
            .expect("measured fps");

        assert!((fps - 29.97).abs() < 0.000001);
        assert_eq!(fps_overlay_text_from_fps(fps).as_deref(), Some("29.97 fps"));
    }

    #[test]
    fn fps_meter_reports_max_speed_over_deterministic_interval() {
        let start = Instant::now();
        let mut meter =
            DisplayFpsMeter::with_min_sample_duration(start, Duration::from_millis(250));

        let fps = meter
            .record_presented_frames(1381, start + Duration::from_secs(10))
            .expect("measured fps");

        assert!((fps - 138.1).abs() < 0.000001);
        assert_eq!(
            fps_overlay_text_from_fps(fps).as_deref(),
            Some("138.10 fps")
        );
    }

    #[test]
    fn fps_meter_uses_initial_estimate_until_first_sample() {
        let start = Instant::now();
        let meter = DisplayFpsMeter::new(start);

        assert_eq!(meter.current_fps(), None);
        assert_eq!(meter.fps_for_overlay(24.0), Some(24.0));
        assert!(DisplayFpsOverlay::from_fps(meter.fps_for_overlay(24.0).unwrap()).is_some());
    }

    #[test]
    fn aspect_fit_letterboxes_wide_source() {
        let rect = aspect_fit_rect(
            DisplaySize {
                width: 3840,
                height: 1600,
            },
        ...
        assert!((rect.y - 140.0).abs() < 0.01);
        assert!((rect.width - 1920.0).abs() < 0.01);
        assert!((rect.height - 800.0).abs() < 0.01);
    }

    #[test]
    fn aspect_fit_16_9_source_fills_1920_by_1080() {
        let viewport = fit_aspect_preserving(1920, 1080, 3840, 2160);

        assert_eq!(viewport.content_x, 0);
        assert_eq!(viewport.content_y, 0);
        assert_eq!(viewport.content_width, 1920);
        assert_eq!(viewport.content_height, 1080);
    }

    #[test]
    fn aspect_fit_4_3_source_pillarboxes_1920_by_1080() {
        let viewport = fit_aspect_preserving(1920, 1080, 4032, 3024);

        assert_eq!(viewport.content_x, 240);
        assert_eq!(viewport.content_y, 0);
        assert_eq!(viewport.content_width, 1440);
        assert_eq!(viewport.content_height, 1080);
    }

    #[test]
    fn aspect_fit_dci_4k_preserves_aspect_into_1920_by_1080() {
        let viewport = fit_aspect_preserving(1920, 1080, 4096, 2160);

        assert_eq!(viewport.content_x, 0);
        assert_eq!(viewport.content_y, 34);
        assert_eq!(viewport.content_width, 1920);
        assert_eq!(viewport.content_height, 1012);
        let source_aspect = 4096.0_f32 / 2160.0_f32;
        let content_aspect = viewport.content_width as f32 / viewport.content_height as f32;
        assert!((source_aspect - content_aspect).abs() < 0.003);
    }

    #[test]
    fn aspect_fit_8k_16_9_fills_uhd_monitor() {
        let viewport = fit_aspect_preserving(3840, 2160, 8192, 4608);

        assert_eq!(viewport.content_x, 0);
        assert_eq!(viewport.content_y, 0);
        assert_eq!(viewport.content_width, 3840);
        assert_eq!(viewport.content_height, 2160);
    }

    #[test]
    fn aspect_fit_portrait_source_pillarboxes() {
        let viewport = fit_aspect_preserving(1920, 1080, 1080, 1920);

        assert_eq!(viewport.content_x, 656);
        assert_eq!(viewport.content_y, 0);
        assert_eq!(viewport.content_width, 607);
        assert_eq!(viewport.content_height, 1080);
    }

    #[test]
    fn aspect_fit_zero_sizes_are_safe() {
        let viewport = fit_aspect_preserving(0, 1080, 4032, 3024);

        assert_eq!(viewport.content_width, 0);
        assert_eq!(viewport.content_height, 0);
        assert_eq!(viewport.scale, 0.0);
    }

    #[test]
    fn aspect_fit_odd_sizes_are_predictable() {
        let viewport = fit_aspect_preserving(1001, 501, 333, 222);

        assert_eq!(viewport.content_width, 751);
        assert_eq!(viewport.content_height, 501);
        assert_eq!(viewport.content_x, 125);
        assert_eq!(viewport.content_y, 0);
    }

    #[test]
    fn frame_pacer_24fps_cadence_is_about_4167_ms() {
        let start = Instant::now();
        let pacer = DisplayFramePacer::new(24.0, start, 0).expect("valid pacer");

        assert!((pacer.frame_duration().as_secs_f64() - (1.0 / 24.0)).abs() < 0.000001);
        assert_eq!(
            pacer
                .next_frame_instant(1)
    ...
    #[test]
    fn frame_pacer_30fps_cadence_is_about_3333_ms() {
        let start = Instant::now();
        let pacer = DisplayFramePacer::new(30.0, start, 0).expect("valid pacer");

        assert!((pacer.frame_duration().as_secs_f64() - (1.0 / 30.0)).abs() < 0.000001);
        assert_eq!(
            pacer
                .next_frame_instant(1)
                .duration_since(start)
    ...
    #[test]
    fn frame_pacer_fast_decode_does_not_advance_before_clock() {
        let start = Instant::now();
        let mut pacer = DisplayFramePacer::new(24.0, start, 0).expect("valid pacer");

        assert_eq!(
            pacer.decision(start, 10),
            DisplayFramePacingDecision::PresentFrame(0)
        );
        pacer.record_presented(0);
        assert_eq!(
            pacer.decision(start + Duration::from_millis(10), 10),
            DisplayFramePacingDecision::WaitUntil(pacer.next_frame_instant(1))
        );
    }

    #[test]
    fn frame_pacer_late_decode_reports_drop_target() {
        let start = Instant::now();
        let mut pacer = DisplayFramePacer::new(24.0, start, 0).expect("valid pacer");
        pacer.record_presented(0);

        assert_eq!(
            pacer.decision(start + Duration::from_millis(130), 10),
            DisplayFramePacingDecision::DropToFrame(3)
        );
    }

    #[test]
    fn frame_pacer_reset_makes_next_frame_due_without_catch_up() {
        let start = Instant::now();
        let frame_duration = Duration::from_millis(33);
        let mut pacer =
            DisplayFramePacer::from_frame_duration(frame_duration, start, 0).expect("valid pacer");
        pacer.record_presented(42);

        let resume = start + Duration::from_secs(5);
        pacer.reset_next_frame_due_now(43, resume);

        assert_eq!(pacer.last_presented_frame(), None);
        assert_eq!(pacer.sleep_duration_until(43, resume), None);
        let next_wait = pacer
            .sleep_duration_until(44, resume)
            .expect("following frame waits");
        assert!(next_wait > frame_duration - Duration::from_millis(1));
        assert!(next_wait <= frame_duration);
    }

    #[test]
    fn transfer_strategy_decodes_shader_srgb_for_srgb_surface() {
        let strategy = DisplayTransferStrategy::for_source_and_surface(
            DisplaySourceTransfer::ShaderSrgb,
            wgpu::TextureFormat::Bgra8UnormSrgb,
        );

        assert!(strategy.present_shader_srgb_decode);
        assert!(!strategy.present_shader_oetf);
        assert!(strategy.double_srgb_avoided);
    }

    #[test]
    fn transfer_strategy_encodes_linear_for_unorm_surface() {
        let strategy = DisplayTransferStrategy::for_source_and_surface(
            DisplaySourceTransfer::HardwareSrgb,
            wgpu::TextureFormat::Bgra8Unorm,
        );
