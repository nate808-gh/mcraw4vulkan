...
impl ProjectionState {
    ...
    fn in_flight_callbacks(&self) -> usize {
        self.in_flight_callbacks.load(AtomicOrdering::SeqCst)
    }

    #[cfg(test)]
    fn cancellation_count(&self) -> usize {
        self.cancelled_commands.load(AtomicOrdering::SeqCst)
    }

    #[cfg(test)]
    fn notification_count(&self) -> usize {
        self.notifications.load(AtomicOrdering::SeqCst)
    }
}
...
mod tests {
    ...
    use crate::{ProviderResult, start_empty_projection};

    #[test]
    fn rejects_non_empty_root() {
        let root = std::env::temp_dir().join(format!(
            "mcraw4vulkan-projfs-ffi-non-empty-{}",
            std::process::id()
        ));
        let mount_root = root.join("mount");
        fs::create_dir_all(&mount_root).unwrap();
        fs::write(mount_root.join("existing.txt"), b"not empty").unwrap();

        let error = match start_empty_projection(&mount_root) {
            Ok(_) => panic!("smoke projection unexpectedly started in a non-empty root"),
            Err(error) => error,
        };

        assert!(error.to_string().contains("root must be empty"));
        let _ = fs::remove_dir_all(root);
    }

    #[test]
    fn fake_provider_returns_directory_and_placeholder_metadata() {
        ...
        assert_eq!(clip_entries.len(), 3);
        let dng_path_string = windows_path(&["clip", "clip_000000.dng"]);
        let dng_path = Path::new(&dng_path_string);
        assert_eq!(
            provider.placeholder_info(dng_path).unwrap().byte_len,
            DNG_BYTES.len() as u64
        );
        assert_eq!(
            provider.read_file(dng_path, 5, 7).unwrap(),
            DNG_BYTES[5..12].to_vec()
        );
        assert_eq!(
            provider.read_file(dng_path, DNG_BYTES.len() as u64 + 10, 16),
            Ok(Vec::new())
        );
        assert_eq!(
    ...
    #[test]
    fn sorts_directory_entries_with_projfs_filename_compare() {
        let entries =
            load_directory_entries(&EnumerationProvider, Path::new("clip"), None).unwrap();
        let names = entry_names(&entries);
        let mut expected = vec![
            OsString::from("z-last.txt"),
            OsString::from("clip_000001.dng"),
            OsString::from("clip.wav"),
            OsString::from("clip_000000.dng"),
        ];
        expected
            .sort_by(|left, right| compare_projfs_os_strings(left.as_os_str(), right.as_os_str()));

        assert_eq!(names, expected);
        assert!(names.windows(2).all(|pair| {
            compare_projfs_os_strings(pair[0].as_os_str(), pair[1].as_os_str()) != Ordering::Greater
        }));
    }

    #[test]
    fn null_search_expression_returns_all_entries() {
        let entries =
            load_directory_entries(&EnumerationProvider, Path::new("clip"), None).unwrap();
    ...
    #[test]
    fn non_wildcard_search_expression_uses_projfs_compare_equality() {
        let entries = load_directory_entries(
            &EnumerationProvider,
            Path::new("clip"),
            Some(OsString::from("clip.wav")),
        )
        .unwrap();

        assert_eq!(entry_names(&entries), vec![OsString::from("clip.wav")]);
    }

    #[test]
    fn wildcard_search_expression_uses_projfs_match() {
        let entries = load_directory_entries(
            &EnumerationProvider,
            Path::new("clip"),
            Some(OsString::from("*.dng")),
        )
        .unwrap();
        let names = entry_names(&entries);
        ...
        assert!(names.contains(&OsString::from("clip_000001.dng")));
    }

    #[test]
    fn restart_scan_resets_cursor_and_recaptures_search_expression() {
        let mut enumeration = DirectoryEnumeration::new(PathBuf::from("clip"));
        let request =
            enumeration.load_request(Path::new("clip"), Some(OsString::from("*.dng")), false);
        assert_eq!(
            request,
            Some((PathBuf::from("clip"), Some(OsString::from("*.dng"))))
        );
        enumeration.replace_entries(vec![entry("clip_000000.dng")]);

        let mut writer = TestDirectoryEntryWriter::new(1);
        assert_eq!(
            fill_directory_entries(&mut enumeration, &mut writer, false),
            S_OK
        );
        assert_eq!(enumeration.next_index, 1);
        assert!(enumeration.completed);

        let restart_request =
            enumeration.load_request(Path::new("clip"), Some(OsString::from("*.wav")), true);

        assert_eq!(
            restart_request,
            Some((PathBuf::from("clip"), Some(OsString::from("*.wav"))))
        );
        assert_eq!(enumeration.next_index, 0);
        assert!(!enumeration.loaded);
        assert!(!enumeration.completed);
        assert!(enumeration.entries.is_empty());
    }

    #[test]
    fn simultaneous_enumeration_sessions_have_independent_cursors() {
        let mut sessions = HashMap::new();
        sessions.insert(1, DirectoryEnumeration::new(PathBuf::from("clip")));
        sessions.insert(2, DirectoryEnumeration::new(PathBuf::from("clip")));
        sessions
            .get_mut(&1)
            .unwrap()
            .replace_entries(vec![entry("a.dng"), entry("b.dng")]);
        sessions
            .get_mut(&2)
            .unwrap()
            .replace_entries(vec![entry("a.dng"), entry("b.dng")]);
        ...
        let mut writer = TestDirectoryEntryWriter::new(1);
        assert_eq!(
            fill_directory_entries(sessions.get_mut(&1).unwrap(), &mut writer, false),
            S_OK
        );

        assert_eq!(sessions.get(&1).unwrap().next_index, 1);
        assert_eq!(sessions.get(&2).unwrap().next_index, 0);
    }

    #[test]
    fn end_directory_enumeration_removes_session_state() {
        let mut sessions = HashMap::new();
        sessions.insert(7, DirectoryEnumeration::new(PathBuf::from("clip")));

        assert!(sessions.remove(&7).is_some());
        assert!(!sessions.contains_key(&7));
    }

    #[test]
    fn insufficient_buffer_after_added_entry_resumes_at_unfit_entry() {
        let mut enumeration = DirectoryEnumeration::new(PathBuf::from("clip"));
        enumeration.replace_entries(vec![entry("a.dng"), entry("b.dng"), entry("c.dng")]);
        let mut writer = TestDirectoryEntryWriter::new(2);
        ...
        assert_eq!(
            writer.written,
            vec![OsString::from("a.dng"), OsString::from("b.dng")]
        );
        assert_eq!(enumeration.next_index, 2);
        assert!(!enumeration.completed);

        let mut writer = TestDirectoryEntryWriter::new(10);
        assert_eq!(
            fill_directory_entries(&mut enumeration, &mut writer, false),
            S_OK
        );
        assert_eq!(writer.written, vec![OsString::from("c.dng")]);
        assert_eq!(enumeration.next_index, 3);
        assert!(enumeration.completed);
    }

    #[test]
    fn insufficient_buffer_before_first_entry_returns_error() {
        let mut enumeration = DirectoryEnumeration::new(PathBuf::from("clip"));
        enumeration.replace_entries(vec![entry("a.dng")]);
        let mut writer = TestDirectoryEntryWriter::new(0);
        let insufficient_buffer = hresult_from_win32(ERROR_INSUFFICIENT_BUFFER);
    ...
    #[test]
    fn stop_without_started_context_is_idempotent() {
        let mut projection = Projection {
            root: PathBuf::new(),
            instance_id: ProjectionInstanceId::from_u128(0),
            context: ptr::null_mut(),
            stopped: false,
            _callbacks: Box::new(callbacks()),
            _state: Box::new(ProjectionState::new(Box::new(FakeProvider))),
        };
        ...
        assert!(projection.context.is_null());
        assert!(projection._state.stopping.load(AtomicOrdering::SeqCst));
        assert_eq!(projection._state.in_flight_callbacks(), 0);
    }

    #[test]
    fn generated_projection_instance_ids_are_unique_per_root() {
        let first_root = windows_fixture_path(&["C:", "mcraw4vulkan", "clip-a"]);
        let second_root = windows_fixture_path(&["C:", "mcraw4vulkan", "clip-b"]);
        let first = generate_projection_instance_id(Path::new(&first_root));
        let second = generate_projection_instance_id(Path::new(&second_root));

        assert_ne!(first, second);
        assert_ne!(first.as_u128(), 0);
        assert_ne!(second.as_u128(), 0);
    }
    ...
    #[test]
    fn projection_instance_id_formats_as_guid_text() {
        let id = ProjectionInstanceId::from_u128(0x12345678_9abc_4def_8123_456789abcdef);
    ...
    #[test]
    fn callback_guard_tracks_in_flight_count_on_success() {
        let state = ProjectionState::new(Box::new(FakeProvider));
        assert_eq!(state.in_flight_callbacks(), 0);

        {
            let _guard = state.try_enter_callback().unwrap();
            assert_eq!(state.in_flight_callbacks(), 1);
        }
    ...
    #[test]
    fn callback_guard_decrements_in_flight_count_on_provider_error() {
        let state = ProjectionState::new(Box::new(FakeProvider));
        let expected = provider_error_to_hresult(ProviderError::NotFound);

        let result = catch_callback(|| {
            let _guard = state.try_enter_callback().unwrap();
            expected
        });
    ...
    #[test]
    fn callback_guard_decrements_in_flight_count_on_panic() {
        let state = ProjectionState::new(Box::new(FakeProvider));

        let result = catch_callback(|| {
            let _guard = state.try_enter_callback().unwrap();
            panic!("intentional callback panic for guard regression test");
        });
    ...
    #[test]
    fn callbacks_after_stopping_return_controlled_failure() {
        let state = ProjectionState::new(Box::new(FakeProvider));
        state.begin_stopping();

        assert_eq!(state.try_enter_callback().err(), Some(E_FAIL));
        assert_eq!(state.in_flight_callbacks(), 0);
    }

    #[test]
    fn cancel_callback_records_synchronous_noop_metric_without_panic() {
        let state = ProjectionState::new(Box::new(FakeProvider));
        let callback_data = callback_data_for_state(&state);

        // SAFETY: callback_data points to a stack value whose InstanceContext
        // references state, and both remain valid for the duration of the call.
        unsafe {
            cancel_command_callback(&callback_data);
        }
    ...
    #[test]
    fn notification_callback_records_metric_without_panic() {
        let state = ProjectionState::new(Box::new(FakeProvider));
        let callback_data = callback_data_for_state(&state);
        ...
        // references state, and both remain valid for the duration of the call.
        let result = unsafe {
            notification_callback(&callback_data, false, 0, ptr::null(), ptr::null_mut())
        };
    ...
    #[test]
    fn load_directory_entries_does_not_take_session_map_lock() {
        let state = ProjectionState::new(Box::new(FakeProvider));
        let _session_lock = state.enumerations.lock().unwrap();

        let entries =
            load_directory_entries(state.provider.as_ref(), Path::new("clip"), None).unwrap();

        assert_eq!(entries.len(), 3);
    }

    #[test]
    fn provider_reads_and_enumerations_can_run_concurrently() {
        let provider = FakeProvider;
        ...
        thread::scope(|scope| {
            for _ in 0..8 {
                ...
                scope.spawn(move || {
                    let entries = load_directory_entries(provider, Path::new("clip"), None)
    ...
    fn callback_data_for_state(state: &ProjectionState) -> PRJ_CALLBACK_DATA {
        PRJ_CALLBACK_DATA {
            InstanceContext: state as *const ProjectionState as *mut c_void,
            ..PRJ_CALLBACK_DATA::default()
        }
    }

    const DNG_BYTES: &[u8] = b"ffi fake virtual DNG bytes: header, ifd, pixel payload, and trailer";
    const WAV_BYTES: &[u8] = b"ffi fake virtual wav bytes";
    const TXT_BYTES: &[u8] = b"ffi fake text sidecar bytes";

    struct EnumerationProvider;

    impl ProjectionProvider for EnumerationProvider {
        fn list_directory(&self, relative_path: &Path) -> ProviderResult<Vec<DirectoryEntry>> {
            if relative_path == Path::new("clip") {
                return Ok(vec![
                    entry("z-last.txt"),
                    entry("clip_000001.dng"),
                    entry("clip.wav"),
                    entry("clip_000000.dng"),
                ]);
            }
    ...
    struct TestDirectoryEntryWriter {
        capacity: usize,
        written: Vec<OsString>,
    }

    impl TestDirectoryEntryWriter {
    ...
    impl DirectoryEntryWriter for TestDirectoryEntryWriter {
        fn write_directory_entry(&mut self, entry: &DirectoryEntry) -> HRESULT {
    ...
    fn entry(name: &'static str) -> DirectoryEntry {
        DirectoryEntry::new(name, PlaceholderInfo::regular_file(1))
    }

    fn entry_names(entries: &[DirectoryEntry]) -> Vec<OsString> {
        entries.iter().map(|entry| entry.name.clone()).collect()
    }

    fn expected_live_clip_entry_names() -> Vec<OsString> {
        let mut names = vec![
            OsString::from("z-last.txt"),
            OsString::from("clip_000000.dng"),
            OsString::from("clip.wav"),
        ];
        names.sort_by(|left, right| compare_projfs_os_strings(left.as_os_str(), right.as_os_str()));
        names
    }

    struct FakeProvider;

    impl ProjectionProvider for FakeProvider {
        fn list_directory(&self, relative_path: &Path) -> ProviderResult<Vec<DirectoryEntry>> {
            ...
            if relative_path == Path::new("clip") {
                return Ok(vec![
                    DirectoryEntry::new(
                        "z-last.txt",
                        PlaceholderInfo::regular_file(TXT_BYTES.len() as u64),
                    ),
                    DirectoryEntry::new(
                        "clip.wav",
                        PlaceholderInfo::regular_file(WAV_BYTES.len() as u64),
                    ),
                    DirectoryEntry::new(
                        "clip_000000.dng",
                        PlaceholderInfo::regular_file(DNG_BYTES.len() as u64),
                    ),
                ]);
            }
        ...
        fn placeholder_info(&self, relative_path: &Path) -> ProviderResult<PlaceholderInfo> {
            if relative_path.as_os_str().is_empty() || relative_path == Path::new("clip") {
                return Ok(PlaceholderInfo::directory());
            }

            if relative_path == Path::new(&windows_path(&["clip", "clip_000000.dng"])) {
                return Ok(PlaceholderInfo::regular_file(DNG_BYTES.len() as u64));
            }

            if relative_path == Path::new(&windows_path(&["clip", "clip.wav"])) {
                return Ok(PlaceholderInfo::regular_file(WAV_BYTES.len() as u64));
            }

            if relative_path == Path::new(&windows_path(&["clip", "z-last.txt"])) {
                return Ok(PlaceholderInfo::regular_file(TXT_BYTES.len() as u64));
            }
        ...
        fn read_file(
            ...
            if relative_path == Path::new(&windows_path(&["clip", "clip_000000.dng"])) {
                return Ok(read_fake_range(DNG_BYTES, byte_offset, length));
            }

            if relative_path == Path::new(&windows_path(&["clip", "clip.wav"])) {
                return Ok(read_fake_range(WAV_BYTES, byte_offset, length));
            }

            if relative_path == Path::new(&windows_path(&["clip", "z-last.txt"])) {
                return Ok(read_fake_range(TXT_BYTES, byte_offset, length));
            }
    ...
    fn read_fake_range(bytes: &[u8], byte_offset: u64, length: u32) -> Vec<u8> {
