...
mod tests {
    ...
    #[test]
    fn sound_error_reports_no_audio_track_for_cli() {
        assert_eq!(
            DisplaySoundError::NoAudioTrack.to_string(),
            "no audio track found for --with-sound"
        );
    }

    #[test]
    fn sdl_audio_spec_uses_track_sample_rate_and_channels() {
        let spec = sdl_audio_spec(stereo_track()).unwrap();

        assert_eq!(spec.sample_rate_hz, 48_000);
        assert_eq!(spec.channels, 2);
        assert_eq!(spec.desired.freq, Some(48_000));
        assert_eq!(spec.desired.channels, Some(2));
        assert_eq!(spec.desired.samples, Some(SDL_AUDIO_BUFFER_SAMPLES));
    }

    #[test]
    fn sdl_audio_spec_rejects_unsupported_audio_shape() {
        let mut track = stereo_track();
        track.bits_per_sample = 24;
        assert!(matches!(
            sdl_audio_spec(track),
            Err(DisplaySoundError::AudioDecode(
                AudioDecodeError::UnsupportedBitsPerSample(24)
            ))
        ));
        ...
        assert!(matches!(
            sdl_audio_spec(track),
            Err(DisplaySoundError::UnsupportedChannelCount(0))
        ));
    }

    #[test]
    fn queued_sample_frames_converts_sdl_byte_count() {
        assert_eq!(queued_sample_frames(19_200, 4), 4_800);
    }

    #[test]
    fn target_queue_duration_uses_sample_rate() {
        assert_eq!(
            sample_frames_for_millis(48_000, TARGET_QUEUE_MILLIS),
            24_000
        );
        assert_eq!(
            sample_frames_for_millis(44_100, TARGET_QUEUE_MILLIS),
            22_050
        );
    }

    #[test]
    fn duration_millis_reports_audio_timeline_length() {
        assert_eq!(duration_millis(96_000, 48_000), 2_000);
    }

    #[test]
    fn frame_to_sample_mapping_uses_aligned_audio_span() {
        assert_eq!(sample_frame_offset_for_frame(0, 100, 48_000), 0);
        assert_eq!(sample_frame_offset_for_frame(50, 100, 48_000), 24_000);
        assert_eq!(sample_frame_offset_for_frame(99, 100, 48_000), 47_520);
    }

    #[test]
    fn frame_to_sample_mapping_clamps_to_valid_frame_range() {
        assert_eq!(sample_frame_offset_for_frame(150, 100, 48_000), 47_520);
        assert_eq!(sample_frame_offset_for_frame(0, 0, 48_000), 0);
        assert_eq!(sample_frame_offset_for_frame(10, 100, 0), 0);
    }
}
