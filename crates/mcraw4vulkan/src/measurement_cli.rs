...
impl InternalPayloadProfile {
    ...
    #[cfg(test)]
    pub fn effective_sink_setting(self) -> &'static str {
        match self {
            Self::DefaultChunked64 => "chunked_offset_prefetch / 64 MiB",
            Self::OffsetPrefetch => "offset_prefetch",
        }
    }
}
...
mod tests {
    use super::*;
    use crate::measurement::types::PayloadReadPolicyMode;

    fn finalize_display(args: &[&str]) -> Result<InternalMeasurementConfig> {
        finalize_for(InternalMeasurementCommand::Display, args)
    }

    fn finalize_pipe(args: &[&str]) -> Result<InternalMeasurementConfig> {
        finalize_for(InternalMeasurementCommand::Pipe, args)
    }

    fn finalize_for(
        command: InternalMeasurementCommand,
        args: &[&str],
    ) -> Result<InternalMeasurementConfig> {
        let args = args
            .iter()
            .map(|value| value.to_string())
            .collect::<Vec<_>>();
        let mut state = InternalMeasurementFlagState::default();
        let mut index = 0usize;
        while index < args.len() {
            index = consume_internal_measurement_flag(&args, index, &mut state)?;
            index += 1;
        }
        Ok(finalize_internal_measurement(command, state)?
            .expect("internal measurement config present"))
    }

    #[test]
    fn omitted_frames_default_to_decision_minimum() {
        let config = finalize_display(&[
            "--internal-measure",
            "--internal-measure-report",
            "report.json",
        ])
        ...
        assert_eq!(config.frames, INTERNAL_MEASUREMENT_DEFAULT_FRAMES);
        assert_eq!(config.frames, OPTIMIZER_DECISION_MIN_FRAMES);
    }

    #[test]
    fn explicit_internal_frames_accept_smoke_decision_and_benchmark_counts() {
        for frames in [1usize, 600, 1200] {
            let config = finalize_display(&[
                "--internal-measure",
                "--internal-measure-report",
                "report.json",
            ...
            .unwrap();
            assert_eq!(config.frames, frames);
        }
    }

    #[test]
    fn zero_internal_frames_reject() {
        let error = finalize_display(&[
            "--internal-measure",
            "--internal-measure-report",
            "report.json",
            "--internal-measure-frames",
        ...
        .expect_err("zero frames rejected")
        .to_string();
    ...
    #[test]
    fn display_and_pipe_default_payload_profile_maps_to_chunked64() {
        for config in [
            finalize_display(&[
                "--internal-measure",
                "--internal-measure-report",
            ...
            let policy = config
                ...
                .payload_read_policy();
            assert_eq!(policy.mode, PayloadReadPolicyMode::ChunkedOffsetPrefetch);
            assert_eq!(policy.chunk_mib, 64);
            assert_eq!(
                config
                    .payload_profile
                    .expect("payload profile")
                    .effective_sink_setting(),
                "chunked_offset_prefetch / 64 MiB"
            );
        }
    }

    #[test]
    fn display_and_pipe_without_explicit_profile_defer_to_settings_resolver() {
        for config in [
            finalize_display(&[
                "--internal-measure",
                "--internal-measure-report",
            ...
            assert_eq!(config.payload_profile, None);
        }
    }

    #[test]
    fn display_and_pipe_alternate_payload_profile_maps_to_offset_prefetch() {
        for config in [
            finalize_display(&[
                "--internal-measure",
                "--internal-measure-report",
            ...
            let policy = config
                ...
                .expect("payload profile")
                .payload_read_policy();
            assert_eq!(policy.mode, PayloadReadPolicyMode::OffsetPrefetch);
            assert_eq!(
                config
                    .payload_profile
                    .expect("payload profile")
    ...
    fn invalid_profiles_reject() {
        assert!(
            ...
            .expect_err("bad payload profile")
            .to_string()
    ...
    #[test]
    fn hidden_flags_require_internal_measure_and_report() {
        let mut state = InternalMeasurementFlagState::default();
        let args = [
            "--internal-measure-report".to_string(),
            "report.json".to_string(),
        ];
        let last = consume_internal_measurement_flag(&args, 0, &mut state).unwrap();
        assert_eq!(last, 1);
        let error = finalize_internal_measurement(InternalMeasurementCommand::Display, state)
            .expect_err("report requires measure")
            .to_string();
        assert!(error.contains("require --internal-measure"));

        let mut state = InternalMeasurementFlagState::default();
        let args = ["--internal-measure".to_string()];
        consume_internal_measurement_flag(&args, 0, &mut state).unwrap();
        let error = finalize_internal_measurement(InternalMeasurementCommand::Display, state)
            .expect_err("measure requires report")
            .to_string();
        assert!(error.contains("requires --internal-measure-report"));
    }

    #[test]
    fn pipe_internal_measurement_policy_uses_canonical_correction_mode() {
        let policy =
            PipeProducerMeasurementPolicy::canonical_discard(GpuBackendPreference::VulkanOnly);
        assert_eq!(
