...
impl DisplayMetrics {
    #[cfg(test)]
    pub fn no_readback(preview_width: u32, preview_height: u32, bytes_per_pixel: u64) -> Self {
        let preview_target_bytes_estimate = u64::from(preview_width)
            .saturating_mul(u64::from(preview_height))
        ...
        Self {
            ...
            readback_kind: ReadbackKind::None,
            output_bytes: 0,
            output_bytes_kind: OutputBytesKind::None,
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn display_proxy_default_is_window_aspect_fit() {
        assert_eq!(
            DisplayProxyPolicy::default(),
            DisplayProxyPolicy::WindowAspectFit {
                width: DEFAULT_DISPLAY_WINDOW_WIDTH,
                height: DEFAULT_DISPLAY_WINDOW_HEIGHT
            }
        );
    ...
    #[test]
    fn aspect_fit_preview_dimensions_match_window_reference() {
        let input = FrameDimensions {
            width: 4032,
            height: 3024,
        };
        let output = DisplayProxyPolicy::default()
            .preview_scale_mode(input)
            .resolve(input)
            .unwrap();
    ...
    #[test]
    fn preview_target_bytes_estimate_uses_rgba8_size() {
        let metrics = DisplayMetrics::no_readback(1440, 1080, 4);
        assert_eq!(metrics.preview_target_bytes_estimate, 1440 * 1080 * 4);
    }

    #[test]
    fn display_metrics_have_no_full_frame_readback_or_output_bytes() {
        let metrics = DisplayMetrics::no_readback(1440, 1080, 4);
        assert!(!metrics.full_frame_gpu_readback_performed);
        assert_eq!(metrics.readback_kind, ReadbackKind::None);
        assert_eq!(metrics.output_bytes, 0);
        assert_eq!(metrics.output_bytes_kind, OutputBytesKind::None);
        assert!(metrics.preview_target_bytes_estimate > 0);
    }

    #[test]
    fn display_mode_labels_are_stable() {
        assert_eq!(
            DisplayMeasurementMode::Cpu.sink().label(),
            "cpu_display_no_vsync_proxy"
        );
        assert_eq!(
            DisplayMeasurementMode::Gpu.sink().label(),
            "gpu_display_no_vsync_proxy"
        );
        assert_eq!(
            DisplayMeasurementMode::GpuVignette.sink().label(),
            "gpu_gpu_vignette_display_no_vsync_proxy"
        );
    }

    #[test]
    fn display_payload_policies_use_production_reorder_window() {
        for mode in [
            PayloadReadPolicyMode::ChunkedOffsetPrefetch,
            PayloadReadPolicyMode::OffsetPrefetch,
        ] {
            let policy = PayloadReadPolicy {
                mode,
                chunk_mib: 64,
            };
            let resolved = policy
                .resolve(&PayloadReadPlan::empty())
                .expect("payload policy resolves");

            assert_eq!(
                resolved.feeder_options.reorder_window,
                PRODUCTION_PAYLOAD_REORDER_WINDOW
            );
            assert_eq!(resolved.feeder_options.reorder_window, 128);
        }
    }

    #[test]
    fn pipe_sink_labels_are_stable() {
        assert_eq!(
            MeasuredSink::GpuGpuVignettePipeYuv444p12LeRaw.label(),
            "gpu_gpu_vignette_pipe_yuv444p12le_raw"
        );
        assert_eq!(
            MeasuredSink::GpuPipeYuv444p12LeRawNoVignette.label(),
            "gpu_pipe_yuv444p12le_raw_no_vignette"
        );
    }

    #[test]
    fn pipe_measurement_policy_uses_canonical_correction_mode() {
        assert_eq!(
    ...
    #[test]
    fn pipe_byte_count_validator_reports_statuses() {
        assert_eq!(validate_pipe_frame_byte_count(10, 10), "OK");
        assert_eq!(validate_pipe_frame_byte_count(10, 9), "LENGTH_MISMATCH");
        assert_eq!(validate_pipe_frame_byte_count(10, 11), "LENGTH_MISMATCH");
    }

    #[test]
    fn pipe_digest_is_deterministic() {
        assert_eq!(pipe_digest64(b"yuv444p12le"), pipe_digest64(b"yuv444p12le"));
        assert_ne!(pipe_digest64(b"yuv444p12le"), pipe_digest64(b"yuv444p12lf"));
    }

    #[test]
    fn pipe_validation_frame_selection_is_bounded_and_stable() {
        let small = [0, 1, 2, 3, 4]
            .into_iter()
            .map(FrameNumber)
            .collect::<Vec<_>>();
        assert_eq!(pipe_validation_frame_indices(&small), vec![0, 1, 2, 3, 4]);
        let large = (0..10).map(FrameNumber).collect::<Vec<_>>();
        assert_eq!(pipe_validation_frame_indices(&large), vec![0, 5, 9]);
        let repeated = [7, 7, 7].into_iter().map(FrameNumber).collect::<Vec<_>>();
        assert_eq!(pipe_validation_frame_indices(&repeated), vec![7]);
    }

    #[test]
    fn pipe_validation_sample_ranges_are_bounded() {
        assert!(pipe_validation_sample_ranges(0).is_empty());
        assert_eq!(pipe_validation_sample_ranges(4), vec![(0, 4)]);
        assert_eq!(
            pipe_validation_sample_ranges(256),
            vec![(0, 64), (128, 64), (192, 64)]
        );
    }
}
