...
mod tests {
    ...
    fn unique_test_path(name: &str) -> PathBuf {
        env::temp_dir().join(format!(
            "mcraw4vulkan-dng-registry-test-{}-{name}.tsv",
            std::process::id()
        ))
    }
    ...
    fn registry_record_round_trips() {
        ...
        let registry = DngMountRegistry::new(path.clone());
        let record = DngMountRegistryRecord::new_with_instance_id(
            PathBuf::from("tmp/source.mcraw"),
            PathBuf::from("tmp/source"),
            1,
            Some("12345678-9abc-4def-8123-456789abcdef".to_string()),
        );

        registry.add_record(record.clone()).unwrap();
        let records = registry.load_records().unwrap();
    ...
    #[test]
    fn registry_reads_v1_rows_without_instance_id() {
        let path = unique_test_path("v1-row");
        let _ = fs::remove_file(&path);
        let registry = DngMountRegistry::new(path.clone());
        fs::write(
            &path,
            format!(
                "{REGISTRY_HEADER_V1}\n1\ttmp/source.mcraw\ttmp/source\t123\t456\twindows\tdng_fuse\tmcraw4vulkan\t123-456-1\n"
            ),
        )
        .unwrap();
        ...
        assert_eq!(records[0].instance_id, None);
        assert!(records[0].is_mcraw4vulkan_owned_dng_mount());
        let _ = fs::remove_file(path);
    }

    #[test]
    fn registry_lookup_by_source_is_exact() {
        let path = unique_test_path("lookup");
        let _ = fs::remove_file(&path);
        let registry = DngMountRegistry::new(path.clone());
        let wanted = DngMountRegistryRecord::new(
            PathBuf::from("tmp/source-a.mcraw"),
            PathBuf::from("tmp/source-a"),
            1,
        );
        let other = DngMountRegistryRecord::new(
            PathBuf::from("tmp/source-b.mcraw"),
            PathBuf::from("tmp/source-b"),
            2,
        );

        registry.add_record(wanted.clone()).unwrap();
        registry.add_record(other).unwrap();

        assert_eq!(
            registry
                .records_for_source(Path::new("tmp/source-a.mcraw"))
                .unwrap(),
            vec![wanted]
        );
        assert!(
            registry
                .records_for_source(Path::new("tmp/source-c.mcraw"))
                .unwrap()
                .is_empty()
        );
        let _ = fs::remove_file(path);
    ...
    #[test]
    fn unmount_all_returns_only_owned_records() {
        let path = unique_test_path("owned");
        let _ = fs::remove_file(&path);
        let registry = DngMountRegistry::new(path.clone());
        let owned = DngMountRegistryRecord::new(
            PathBuf::from("tmp/source-a.mcraw"),
            PathBuf::from("tmp/source-a"),
            1,
        );
        let mut unrelated = DngMountRegistryRecord::new(
            PathBuf::from("tmp/source-b.mcraw"),
            PathBuf::from("tmp/source-b"),
            2,
        );
        ...
        assert_eq!(registry.all_owned_records().unwrap(), vec![owned]);
        let _ = fs::remove_file(path);
    }

    #[test]
    fn registry_removes_selected_mount_ids() {
        let path = unique_test_path("remove");
        let _ = fs::remove_file(&path);
        let registry = DngMountRegistry::new(path.clone());
        let first = DngMountRegistryRecord::new(
            PathBuf::from("tmp/source-a.mcraw"),
            PathBuf::from("tmp/source-a"),
            1,
        );
        let second = DngMountRegistryRecord::new(
            PathBuf::from("tmp/source-b.mcraw"),
            PathBuf::from("tmp/source-b"),
            2,
        );
        ...
        registry
            ...
            .unwrap();
        registry
            .remove_mount_ids(std::slice::from_ref(&first.mount_id))
            .unwrap();

        assert_eq!(registry.load_records().unwrap(), vec![second]);
        let _ = fs::remove_file(path);
    }
}
