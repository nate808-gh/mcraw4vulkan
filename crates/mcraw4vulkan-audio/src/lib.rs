...
#[cfg(test)]
mod tests {
    use mcraw4vulkan_core::{AudioTrackInfo, ClipTimingInfo, FrameRate};
    use mcraw4vulkan_mcrawcontainer::AudioSyncInfo;

    use super::{
        AudioByteRange, AudioDecodeError, AudioVideoTimelineRequest, TimelineAudioAlignmentInfo,
        align_audio_samples_to_playback_timeline, bytes_per_sample_frame,
        plan_audio_alignment_for_playback, plan_audio_video_timeline,
        sample_frame_range_for_pcm_byte_range, timeline_sample_frames_for_duration_us,
    };

    fn stereo_track() -> AudioTrackInfo {
        AudioTrackInfo {
    ...
    fn video_timing(frame_count: u64, frame_rate: FrameRate) -> ClipTimingInfo {
        ClipTimingInfo::from_sources(frame_count, Some(frame_rate), None, None, None, frame_rate)
    }

    fn one_frame_trim_sync() -> AudioSyncInfo {
        AudioSyncInfo {
            first_video_timestamp_ns: 0,
            first_audio_timestamp_ns: -1,
            drift_ns: -1,
            sample_frames_to_trim: 1,
            sample_frames_to_insert_silence: 0,
            sync_is_within_one_second: true,
        }
    }

    #[test]
    fn computes_bytes_per_sample_frame_for_s16le() {
        assert_eq!(bytes_per_sample_frame(stereo_track()).unwrap(), 4);
    }

    #[test]
    fn rejects_unsupported_audio_shape() {
        let mut track = stereo_track();
        track.bits_per_sample = 24;

        assert!(matches!(
            bytes_per_sample_frame(track),
            Err(AudioDecodeError::UnsupportedBitsPerSample(24))
        ));
        ...
        assert!(matches!(
            bytes_per_sample_frame(track),
            Err(AudioDecodeError::InvalidChannelCount)
        ));
    }

    #[test]
    fn maps_unaligned_byte_ranges_to_covering_sample_frames() {
        let range = sample_frame_range_for_pcm_byte_range(
            stereo_track(),
            AudioByteRange { offset: 3, len: 8 },
        )
        .unwrap();

        assert_eq!(range.start_sample_frame, 0);
        assert_eq!(range.sample_frame_count, 3);
    }

    #[test]
    fn rounds_timeline_duration_like_decoder() {
        assert_eq!(
            timeline_sample_frames_for_duration_us(2_266_741, 48_000),
            Some(108_804)
        );
        assert_eq!(
            timeline_sample_frames_for_duration_us(1_700_148, 48_000),
            Some(81_607)
        );
    }

    #[test]
    fn plans_tail_silence_when_audio_is_shorter_than_playback_timeline() {
        let info = plan_audio_alignment_for_playback(stereo_track(), 4, None, 125).unwrap();

        assert_eq!(
            info,
            TimelineAudioAlignmentInfo {
                raw_sample_frames: 4,
                start_synced_sample_frames: 4,
                target_timeline_sample_frames: 6,
                output_sample_frames: 6,
                leading_trim_sample_frames: 0,
                leading_silence_sample_frames: 0,
                tail_trim_sample_frames: 0,
                tail_silence_sample_frames: 2,
                target_duration_us: 125,
            }
        );
    }

    #[test]
    fn plans_tail_trim_when_audio_is_longer_than_playback_timeline() {
        let info = plan_audio_alignment_for_playback(stereo_track(), 8, None, 125).unwrap();

        assert_eq!(info.target_timeline_sample_frames, 6);
        assert_eq!(info.start_synced_sample_frames, 8);
        assert_eq!(info.tail_trim_sample_frames, 2);
        assert_eq!(info.tail_silence_sample_frames, 0);
    }

    #[test]
    fn plans_no_tail_adjustment_when_audio_matches_playback_timeline() {
        let info = plan_audio_alignment_for_playback(stereo_track(), 6, None, 125).unwrap();

        assert_eq!(info.target_timeline_sample_frames, 6);
        assert_eq!(info.start_synced_sample_frames, 6);
        assert_eq!(info.tail_trim_sample_frames, 0);
        assert_eq!(info.tail_silence_sample_frames, 0);
    }

    #[test]
    fn plans_silence_for_zero_source_samples_with_positive_timeline() {
        let info = plan_audio_alignment_for_playback(stereo_track(), 0, None, 125).unwrap();

        assert_eq!(info.raw_sample_frames, 0);
        assert_eq!(info.start_synced_sample_frames, 0);
        assert_eq!(info.target_timeline_sample_frames, 6);
        assert_eq!(info.tail_silence_sample_frames, 6);
    }

    #[test]
    fn plans_sample_style_audio_trim_from_playback_duration() {
        let frame_rate = FrameRate::new(62_500_000, 2_606_379);
        let playback_duration_us = frame_rate.duration_us_for_frame_count(16_167).unwrap();
        let mut track = stereo_track();
        track.total_samples = 32_361_600;

        let info = plan_audio_alignment_for_playback(
            track,
            track.total_samples,
            None,
            playback_duration_us,
        ...
        assert_eq!(playback_duration_us, 674_197_268);
        assert_eq!(info.target_timeline_sample_frames, 32_361_469);
        assert_eq!(info.tail_trim_sample_frames, 131);
        assert_eq!(info.tail_silence_sample_frames, 0);
    }

    #[test]
    fn helper_and_mutating_alignment_agree() {
        let mut samples = vec![1, 2, 3, 4, 5, 6, 7, 8];
        let sync = one_frame_trim_sync();
        let planned =
            plan_audio_alignment_for_playback(stereo_track(), 4, Some(sync), 125).unwrap();
        let applied = align_audio_samples_to_playback_timeline(
            &mut samples,
            stereo_track(),
            4,
            Some(sync),
        ...
        assert_eq!(samples, vec![3, 4, 5, 6, 7, 8, 0, 0, 0, 0, 0, 0]);
    }

    #[test]
    fn plans_audio_video_timeline_with_and_without_audio() {
        let timing = video_timing(24, FrameRate::integer(24));
        let with_audio = plan_audio_video_timeline(AudioVideoTimelineRequest {
            timing,
            video_frame_count: 24,
            audio_track: Some(stereo_track()),
            audio_sync_info: None,
        })
        .unwrap();

        assert!(with_audio.audio_is_present());
        assert_eq!(with_audio.playback_frame_rate, FrameRate::integer(24));
        assert_eq!(with_audio.playback_duration_us, 1_000_000);
        assert_eq!(
            with_audio
                .audio_alignment
                .unwrap()
        ...
        let without_audio = plan_audio_video_timeline(AudioVideoTimelineRequest {
            timing,
            video_frame_count: 24,
            audio_track: None,
            audio_sync_info: None,
        ...
        assert!(!without_audio.audio_is_present());
        assert!(without_audio.audio_alignment.is_none());
    }

    #[test]
    fn aligns_audio_samples_with_trim_and_tail_silence() {
        let mut samples = vec![1, 2, 3, 4, 5, 6, 7, 8];
        let info = align_audio_samples_to_playback_timeline(
            &mut samples,
            stereo_track(),
            4,
            Some(one_frame_trim_sync()),
            125,
        )
        .unwrap();
        ...
        assert_eq!(
            ...
            TimelineAudioAlignmentInfo {
                raw_sample_frames: 4,
                start_synced_sample_frames: 3,
                target_timeline_sample_frames: 6,
                output_sample_frames: 6,
                leading_trim_sample_frames: 1,
                leading_silence_sample_frames: 0,
                tail_trim_sample_frames: 0,
                tail_silence_sample_frames: 3,
                target_duration_us: 125,
            }
        );
    }
